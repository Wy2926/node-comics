...
try {
  ...
  await page.reload();
  const {ZipWriter,Uint8ArrayReader,Uint8ArrayWriter}=createRequire(path.resolve('apps/extension/package.json'))('@zip.js/zip.js');
  const archive=new ZipWriter(new Uint8ArrayWriter());for(const i of [1,2,3])await archive.add(`page-${i}.png`,new Uint8ArrayReader(await readFile(path.join(process.env.UI_FIXTURE_DIRECTORY,'pages',`page-0${i}.png`))));
  await page.locator('input[type=file]').setInputFiles({name:'R2 验收.cbz',mimeType:'application/zip',buffer:Buffer.from(await archive.close())});
  await completeLocalImport(page);await page.getByRole('button',{name:'继续阅读',exact:true}).first().click();
