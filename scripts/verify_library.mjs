...
try{
 ...
 dialog=page.getByRole('dialog');await dialog.getByRole('group',{name:'合订收录',exact:true}).getByRole('button').nth(0).click();await dialog.getByRole('group',{name:'合订收录',exact:true}).getByRole('button').nth(1).click();await dialog.getByRole('button',{name:'保存修改',exact:true}).click();await dialog.locator('.nc-publication-contents[open]').waitFor();
 assert.equal(await dialog.getByRole('region',{name:'合订收录',exact:true}).getByRole('listitem').count(),2);await dialog.getByRole('button',{name:'关闭弹窗',exact:true}).click();
