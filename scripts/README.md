...
| `verify_source_architecture.mjs` | 新来源基线：120 页 CBZ 完整容器导入、跳第 100 页、关闭浏览器重开、4 类缓存清理和窄屏；默认 Vite 5175，`TEST_EXTENSION=1` 改为解压扩展 |
| `verify_source_database_baseline.mjs` | 在隔离扩展 profile 中先创建残缺旧 v1 库，再验证新基线导入、阅读、重开且旧库不变；另检查当前基线缺表的明确错误，无未处理 Promise |
| `verify_drive_import.mjs` | 新建隔离扩展 profile、专用 HTTPS 测试页与模拟 Google 服务，覆盖选图、导入、读页和授权复用；`TEST_DRIVE_AUTH_MODE=chrome` 增加模拟 Chrome Identity、真实浏览器重启后的静默恢复和断开检查，不读取用户 Chrome 或真实云盘，见 [Drive 授权回归](../docs/validation/DRIVE_AUTH_2026_09_23.md) |
| `verify_source_formats.mjs` | 新基线解压扩展：实际 PDF、MOBI、RAR4 / RAR5 样本导入和页面解码；先生成样本并构建 Chrome 扩展 |
| `verify_source_export.mjs` | 新基线解压扩展：完整源文件逐字节导出、3 页 CBZ 与 PDF 可重新解析；仅自制样本与本机下载 |
| `verify_website_source_lifecycle.mjs` | 本机 TLS / 隔离 DNS 下操作真实网站嵌入按钮、插件确认页、来源标签页、按需取图和主动下载；重开并让图片源失败后已下载页仍可读，见[本轮验收](../docs/validation/SOURCE_ARCHITECTURE_2026_09_22.md) |
...
| `verify_extension_theme.mjs` | 新书架、真实导入、修改 / 导出弹窗、4 色 × 亮暗主题、阅读浮层和窄屏；默认 Vite 5175，可用 `TEST_READER_URL` 指定隔离主应用 |
...
node scripts/verify_source_architecture.mjs
node scripts/verify_source_database_baseline.mjs
node scripts/verify_source_formats.mjs
...
普通网页检查：在扩展目录以 `npx vite --host 127.0.0.1 --port 5175 --strictPort` 启动独立 Vite，移除 `TEST_EXTENSION` 或设为 `0`，再运行 `verify_source_architecture.mjs`；可通过 `TEST_READER_URL` 指定该隔离来源。扩展检查读取 `.output/chrome-mv3`，不需要 Vite。结果与真实运行范围见[本轮总验收](../docs/validation/SOURCE_ARCHITECTURE_2026_09_22.md)。

Drive 完整交互回归沿用上述 Playwright / 浏览器变量和自制 `1.png`，需要 Python `cryptography`。构建扩展时须配置 `VITE_DRIVE_CONNECT_URL`（HTTPS 的 `/drive-connect/index.html`）；Chrome 托管授权构建另设 `VITE_GOOGLE_CHROME_CLIENT_ID`，两种客户端配置见 [drive-connect](../apps/drive-connect/README.md)。脚本读取构建中的地址，在新建 profile 的隔离 DNS 内将授权页、GIS / Picker 与 Drive API 全部映射到本机 TLS 夹具，不访问真实 Google 或用户 profile，也不读取实际授权页配置。
...
作品管理回归使用同一组 Playwright / Chrome for Testing 环境变量，先构建扩展，再执行 `node scripts/verify_work_management.mjs`。脚本在新建的隔离 profile 中生成 CBZ，验证作品及条目编辑、分类筛选、已读、排序、版本选择与阅读位置、封面、归属修正、跨书架页管理与导入、窄屏及卡片操作收纳。它拦截外部 HTTP 请求，不访问用户书架或真实账号；截图与结果保存至 `artifacts/work-management/run-*/`。交互决策见 [作品管理与导入交互](../docs/WORK_MANAGEMENT_UX.md)。

旧 `verify_acquisition_order.mjs`、`verify_local_import.mjs`、`verify_comic_import.mjs`、`verify_library.mjs`、`verify_comic_export.mjs`、`verify_web_import.mjs`、`verify_mangacopy.mjs` 和 `inspect_mobi.mjs` 已随旧库基线移除；历史文档中的旧命令不是当前运行入口。MangaCopy 当前本地验收使用 `verify_website_source_lifecycle.mjs`，不访问公开站点。
