"""Offline CJK/Latin acceptance for the complete pinned Qt typesetter."""
import asyncio
import base64
...
async def main():
    ...
    initialize(DictionaryStore(default_directory(), ['en', 'ja', 'zh-Hans', 'zh-Hant']))
    sheet = Image.new('RGB', (840, 2000), 'white')
    ...
    with patch('urllib.request.urlopen', side_effect=AssertionError('Must render offline')), \
         patch('requests.get', side_effect=AssertionError('Must render offline')):
        for index, (language, translation) in enumerate(samples.items()):
            ...
            labels.text((430,index*500+4),'Manga Translator UI: full Qt layout',fill='black')
            results.append({'language':language,'bubbles':result['bubble_regions'],
                            'fitted':result['layout_fitted_regions'],'glyph_pixels':int(glyphs.sum()),'layout':result['layout']})
    sheet.save(destination/'overview.png')
    report={'kind':'synthetic-lettering-only','network':False,'detector':'BallonsTranslator@84ba500',
            'renderer':'manga-translator-ui@f0307a0', 'results':results}
