...
try{
  ...
  const deepLink=await context.newPage();await deepLink.goto(home+'#search');await deepLink.locator('.nc-search-page').waitFor();assert.equal(new URL(deepLink.url()).hash,'#search');assert.equal(await deepLink.locator('dialog[open]').count(),0);await deepLink.close();check('A fresh #search URL opens the standalone page without a comic seed');
