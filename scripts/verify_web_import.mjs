...
try{
 ...
 await setSourceImages(['5']);p=await popup();let insert=await openImport(p);await insert.getByRole('button',{name:'插入已有副本',exact:true}).click();await insert.getByLabel('目标作品',{exact:true}).selectOption(workId);await insert.getByLabel('目标副本',{exact:true}).selectOption(copyId);await insert.getByLabel('插入位置',{exact:true}).selectOption('after');await insert.getByLabel('接在哪一页后',{exact:true}).selectOption(originalPageIds[0]);
 await insert.getByRole('dialog').screenshot({path:path.join(output,'insert-desktop.png')});await insert.setViewportSize({width:390,height:844});await insert.getByRole('dialog').screenshot({path:path.join(output,'insert-mobile.png')});assert(await insert.getByRole('dialog').evaluate(node=>node.scrollWidth<=node.clientWidth));await insert.setViewportSize({width:1440,height:1000});
 ...
 for(const domain of ['copy4000.com','www.copy4000.com','mangacopy.com','www.mangacopy.com']){
  ...
  const catalog=await settingsPage.evaluate(async tabId=>chrome.runtime.sendMessage({type:'NC_DISCOVER_TAB',tabId}),tabId);assert(catalog.ok);assert.equal(catalog.data.catalog.id,'mangacopy:sample');assert.equal(new URL(catalog.data.catalog.url).hostname,domain);
 ...
 const catalog={id:'mangacopy:redirect_sample',sourceId:'mangacopy',url:'https://www.mangacopy.com/comic/redirect_sample',title:'镜像跳转样本',entries:[{id:'redirect-entry',url:`https://www.mangacopy.com/comic/redirect_sample/chapter/${uuid}`}],groups:[],complete:true,observedAt:Date.now(),note:'',excludedEntryIds:[]};
