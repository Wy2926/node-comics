...
try{
 ...
 for(const [theme,label] of [['sky','晴空蓝'],['rose','樱花粉'],['mint','薄荷绿'],['iris','鸢尾紫'],['amber','琥珀橙'],['slate','石墨灰']])for(const mode of ['light','dark']){
  ...
  const panelStyle=element=>{const style=getComputedStyle(element);return {border:style.borderTop,borderRadius:style.borderRadius,shadow:style.boxShadow};};
  const sharedPanel=await page.locator('.settings-card').first().evaluate(panelStyle);
  await page.getByRole('navigation',{name:'主导航'}).getByRole('button',{name:'搜索漫画',exact:true}).click();
  await page.getByRole('button',{name:'翻译名称搜索',exact:true}).click();
  assert.deepEqual(await page.locator('.nc-search-composer').evaluate(panelStyle),sharedPanel,'Search composer must follow the shared panel theme');
  assert.deepEqual(await page.locator('.nc-search-sites').evaluate(panelStyle),sharedPanel,'Search scope must follow the shared panel theme');
  const inputBorder=await page.locator('.nc-search-name input').evaluate(element=>getComputedStyle(element).borderTop);
  assert.equal(await page.getByRole('combobox',{name:'名称目标语言',exact:true}).evaluate(element=>getComputedStyle(element).borderTop),inputBorder);
  await noOverflow();await shot('search-'+theme+'-'+mode);
  await page.getByRole('button',{name:'外观与设置',exact:true}).click();
 }
 await selectOption(page.getByRole('combobox',{name:'界面文字大小',exact:true}),'1.25');await noOverflow();await shot('settings-large-text');
