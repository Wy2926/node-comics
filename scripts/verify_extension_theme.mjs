...
try{
 ...
 const imports=page.getByRole('dialog',{name:'导入本地漫画'});await shot('import-light');await imports.getByRole('button',{name:/^开始导入/}).click();await imports.locator('.nc-import-item.created').waitFor({timeout:60000});await imports.getByRole('button',{name:'完成',exact:true}).click();
 await page.locator('.nc-book img').waitFor();await shot('library-light');assert.equal(await page.getByRole('button',{name:/翻译队列|翻译记录|网站来源/}).count(),0);checks.push('作品架与导入面板使用新来源模型，不新增网站导入按钮');
 const search=page.getByRole('searchbox',{name:'搜索当前页作品'});await search.focus();assert.equal(await search.evaluate(el=>getComputedStyle(el).outlineStyle),'none');await shot('library-input-focus');await search.blur();
 await page.getByRole('button',{name:'目录',exact:true}).click();await shot('work-light');await page.getByRole('button',{name:'修改标题',exact:true}).click();await page.getByRole('dialog').waitFor();await shot('editor-light');await page.getByRole('button',{name:'关闭弹窗',exact:true}).click();
 await page.getByRole('button',{name:'导出',exact:true}).click();await page.getByRole('dialog').getByRole('combobox').first().click();await page.getByRole('listbox').waitFor();await shot('export-dropdown');await page.keyboard.press('Escape');await page.getByRole('button',{name:'关闭弹窗',exact:true}).click();checks.push('作品修改与导出弹窗、顶层下拉菜单');
 ...
 await page.getByRole('button',{name:'返回我的漫画',exact:true}).click();await page.getByRole('button',{name:'开始阅读',exact:true}).click();await page.locator('.nc-page-image').first().waitFor();await shot('reader-light');
