...
try {
  ...
  await page.evaluate(()=>{
    ...
    window.fetch=async(input,init)=>{
      ...
      if(url.pathname.includes('translation-batches'))return json({items:[0,1].map(n=>({id:`fixture-job-${n}`,input_asset_id:`asset-${n}`,output_asset_id:state.status==='succeeded'&&!(n===0&&state.deleted)?`output-${n}`:null,result_available:state.status==='succeeded'&&!(n===0&&state.deleted),result_expired:n===0&&state.deleted,batch_id:'history-0',reused:false,mode:'classic',target_language:'zh-Hans',status:state.status,phase:'translating_text',cost:1,settlement:state.status==='running'?'reserved':'settled',version:1,cache_hit:false,created_at:'2026-09-15T00:00:00Z'})),total:2,next_offset:null});
