...
async function imports(slug,offline){
  ...
  await reader.getByRole('button',{name:'下一步 · 确认归属',exact:true}).click();
  if(offline)await reader.getByRole('button',{name:/^离线保存/}).click();
  await reader.getByRole('button',{name:'确认导入 1 项',exact:true}).click();
  await reader.getByRole('button',{name:'打开作品 '+(offline?'网站离线验收':'网站按需验收'),exact:true}).waitFor();
  await source.close();checks.push('Embedded website button opens '+(offline?'explicit download':'on-demand')+' confirmation');
}
...
async function cacheCount(name){return reader.evaluate(name=>new Promise((resolve,reject)=>{const open=indexedDB.open('node-comics-sources-v1-'+name);open.onerror=()=>reject(open.error);open.onsuccess=()=>{try{const db=open.result,count=db.transaction('metadata').objectStore('metadata').count();count.onsuccess=()=>{resolve(count.result);db.close();};count.onerror=()=>reject(count.error);}catch(error){reject(error);}};}),name);}
...
try{
  ...
  await imports('ondemand',false);let value=await state();assert.equal(value.tasks.length,0);assert.equal(value.pages.length,0);assert.equal(extensionImageReads,0);
  await reader.getByRole('button',{name:'开始阅读',exact:true}).click();await rendered();
  ...
  const retainedId=value.tasks.find(task=>task.status==='complete').documentId;checks.push('Explicit download discovers and retains both pages after the managed source tab closes');
  await reader.screenshot({path:path.join(out,'retained-download.png')});
  await reader.getByRole('button',{name:'外观与设置',exact:true}).click();
  ...
  const beforeReads=imageRequests.filter(url=>url.startsWith('/retained/')).length;const card=reader.locator('article.nc-book').filter({has:reader.getByRole('button',{name:'打开作品 网站离线验收',exact:true})});await card.getByRole('button',{name:'开始阅读',exact:true}).click();await rendered();
  assert.equal(imageRequests.filter(url=>url.startsWith('/retained/')).length,beforeReads);assert((await state()).tasks.some(task=>task.documentId===retainedId&&task.status==='complete'));await reader.screenshot({path:path.join(out,'retained-offline-after-restart.png')});checks.push('Browser restart reads retained pages while source responses fail, without another retained-source fetch');
