...
let local: Record<string, unknown>, session: Record<string, unknown>, snapshot: PageManifest;
...
beforeEach(() => {
  ...
  snapshot = { id: 'untrusted-content-id', sourceTabId: 999, navigationId: 'navigation', revision: 1, title: 'Test', url: 'https://example.org/chapter', adapter: 'test', direction: 'rtl', discoveryComplete: true, note: '', items: [{ id: 'page-1', url: 'https://images.example.org/1.png', width: 800, height: 1200, order: 0 }] };
...
describe('persisted source manifest authority', () => {
  it('registers managed discoveries under a background ID and resolves HTTP pages after their tab closes', async () => {
    ...
    expect(discovered).toMatchObject({ ok: true, data: { id: 'trusted-manifest', sourceTabId: 7 } });
  ...
  it('keeps a stable managed manifest while discovery grows', async () => {
    await send({ type: 'NC_POLL_SOURCE', tabId: 7 }); snapshot.items.push({ ...snapshot.items[0], id: 'page-2', url: 'https://images.example.org/2.png', order: 1 });
