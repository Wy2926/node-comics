...
it('allows fresh estimation after the server definitively rejected an expired preview',()=>{expect(submissionRejected(new ApiError('expired','PREVIEW_EXPIRED',409))).toBe(true);expect(submissionRejected(new ApiError('changed','PREVIEW_CHANGED',409))).toBe(true);});
