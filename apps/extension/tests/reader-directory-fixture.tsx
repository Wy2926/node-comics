...
function Fixture(){
 ...
 const directory=useMemo<ReadingDirectory>(()=>({title:'620 章 · 目录定位验收',sourceUrl:'https://fixture.invalid/comic',entries:copies.map(item=>({id:item.id,title:item.title,tags:[],current:item.id===id,read:false,readable:true,total:item.pages.length,status})),chapters:copies.map((item,index)=>({id:item.id,title:item.title,entryIds:[item.id],selectedEntryId:item.id,groupIds:grouped?['group-'+Math.floor(index/100)]:[],current:item.id===id,readable:true})),groups:grouped?[{id:'root',title:'来源分类',entryIds:[]},...Array.from({length:7},(_,n)=>({id:'group-'+n,parentId:'root',title:`分组 ${n+1}`,entryIds:copies.slice(n*100,(n+1)*100).map(item=>item.id)}))]:[]}),[copies,id,grouped,status]);
 ...
 return <div className="nc-app" style={{height:'100vh',display:'flex',flexDirection:'column'}}>
  <div style={{padding:8,display:'flex',gap:12,background:'white',zIndex:20}}>
   ...
   <button data-directory-revision={status==='状态已刷新'?1:0} onClick={()=>setStatus(value=>value==='可以阅读'?'状态已刷新':'可以阅读')}>刷新目录状态</button>
