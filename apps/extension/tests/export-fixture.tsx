...
const works=await catalog.list('comics',{limit:100});
if(works.some(work=>!work.title.startsWith('导出验收')))throw Error('此隔离来源已有其他资料，停止写入夹具。');
let exportedDocument=(await catalog.list('entries',{limit:1}))[0];
if(!exportedDocument){
  ...
  const result=await importLocalFile(new File([await writer.close()],'导出验收样本.cbz'));
  exportedDocument=(await catalog.get('entries',result.id))!;
}
