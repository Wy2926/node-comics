...
import {defaults,type Job,type Entitlements,type TranslationPlan,type TranslationOperation} from '../src/types';
...
const {copies:stored,ordinals}=await seedReaderFixture(origin,autoScenario,job);
...
window.fetch=async(input,init={})=>{
  ...
  if(url.pathname==='/v1/translation-plans'&&init.method==='POST'){
    ...
    const items:TranslationOperation[]=value.items.map(item=>{
      ...
      const image=item.image,index=ordinals[image.client_item_id];
      if(index===undefined)throw Error('Unknown fixture page identity.');
      ...
      const j:Job={...job(index,image.asset_id?'queued':'awaiting_upload'),id:'submitted-'+crypto.randomUUID(),mode:item.mode,target_language:item.target_language,image_sha256:image.image_sha256,created_at:new Date().toISOString(),input_asset_id:image.asset_id??null};
      jobs.set(j.id,j);state.submitted.push(index);
  ...
  if(url.pathname==='/v1/me/translation-changes'){
    for(const j of jobs.values()){
      if(j.status==='validating_upload'){j.status=autoScenario?'running':'queued';j.input_asset_id='asset-'+j.id;}
