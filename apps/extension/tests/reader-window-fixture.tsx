...
import {defaults,type ReadingCopy,type Settings} from '../src/types';
...
function Fixture(){
 ...
 const update=useCallback((next:ReadingCopy)=>setCopies(previous=>previous.map(value=>value.id===next.id?next:value)),[]);
 ...
 return <div className="nc-app" style={{height:'100vh',display:'flex',flexDirection:'column'}}>
  ...
  {mounted&&<Reader key={mounted?'reader':'closed'} viewKey="isolated-window-fixture" copy={copy} sequence={copies} settings={config} setSettings={setConfig} update={update} onActiveCopy={active} onLoadCopy={noop} onMarkRead={mark} onNavigate={navigate} onBack={()=>setMounted(false)} onRetry={noop} onUpgrade={noop} onLogin={noop} translationState={()=>undefined} onImport={()=>setStatus('本地样本导入入口')} notify={setStatus} onReadingWindow={noop} apiOrigin={location.origin} api={api} busy={false} onAcquire={noop} onPauseAcquire={noop}/>}
 </div>;
}
