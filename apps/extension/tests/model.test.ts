...
import {activeWindow,anchorFor,naturalSort} from '../src/reader/model';
import {safeImageUrl} from '../src/sources/adapters';
describe('bounded reader and source boundary',()=>{
it('keeps a five-page decode window even in a 100 page chapter',()=>{expect(activeWindow(50,100)).toEqual({start:48,end:52});expect(activeWindow(0,100)).toEqual({start:0,end:2});expect(activeWindow(99,100)).toEqual({start:97,end:99});});
