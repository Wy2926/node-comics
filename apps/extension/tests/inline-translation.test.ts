...
describe('in-page translation',()=>{
 ...
 it.each(['queued','running','failed','cancelled','outcome_unknown','unknown_released','no_text','succeeded'] as const)('shares reader state for %s',status=>{const page={...target(0).page,translationScope:JSON.stringify([origin,'reader']),jobs:[job(0,{status})]};const state=translationState({page,mode:'classic',language:'zh-Hans',userId:'reader',origin,active:true});if(status==='queued')expect(state?.kind).toBe('waiting');if(status==='unknown_released')expect(state?.retryable).toBe(false);});
 it('keeps classic fallback independent of a redraw attempt and rejects another account result',()=>{const page={...target(0).page,translationScope:JSON.stringify([origin,'reader']),jobs:[job(0,{status:'succeeded',output_asset_id:'output'})]};expect(pageTranslation(page,'classic','zh-Hans',JSON.stringify([origin,'reader'])).result?.output_asset_id).toBe('output');expect(pageTranslation(page,'classic','zh-Hans',JSON.stringify([origin,'other'])).result).toBeUndefined();});
});
