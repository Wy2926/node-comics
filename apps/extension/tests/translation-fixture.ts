...
export const target=(n:number)=>({copyId:'book',mode:'classic' as const,page:{...emptyPage(n+'.png',800,1200),id:'page-'+n,fileHash:'a'.repeat(64),pageIndex:n,imageSha256:originalSha(n),imageByteSize:originalBytes(n).size,imageMime:'image/png',blobKey:'blob-'+n}});
...
export const receipt=(body:TranslationPlan):PlanReceipt=>({policy_revision:'1',image_rate_limit:{window_seconds:60,limit:10,remaining:9},items:body.items.map(item=>({operation_key:item.operation_key,page_key:item.page_key,disposition:'accepted',job:job(Number(item.image.client_item_id.replace('page-','')))}))});
export function fixture(){
 const api=new Api(origin),userId=crypto.randomUUID(),rights=entitlement(),onJobs=vi.fn(async(_jobs:Job[])=>{});
 const core=new TranslationCoordinator({api,userId,language:'zh-Hans',sessionId:crypto.randomUUID(),getBlob:async key=>originalBytes(Number(key.replace('blob-',''))),rights:()=>rights,onJobs,onChange:()=>{}});
