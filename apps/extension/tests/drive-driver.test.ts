...
function context(): OpenFileSourceContext {
  return {connection,entryId:'entry',contentId:'content',format:'cbz',sourceSnapshot:{...snapshot},
   source:{connectionId:connection.id,providerItemId:'file-1',locator:{...snapshot,version:'99'},generation:1,status:'active'}};
}
...
describe('Google Drive file source driver', () => {
  ...
  it.each([
    ...
    const input = context(); input.sourceSnapshot = value;
  ...
  it('rejects a frozen revision from another account or another source item', async () => {
    const foreign = context(); foreign.sourceSnapshot!.accountId = 'account-2';
    await expect(googleDriveDriver.open(foreign)).rejects.toMatchObject({code: 'account-mismatch'});
    const item = context(); item.source.providerItemId = 'other-file';
...
it('rejects a Drive image even if a picker response bypasses file filtering',async()=>{client.chooseDriveFiles.mockResolvedValue({account:{id:'account-1'},files:[{...snapshot,name:'page.png',format:'image'}]});await expect(googleDriveDriver.select!()).rejects.toMatchObject({code:'unsupported-format'});});
