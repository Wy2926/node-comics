...
it.each([false,true])('registers Firefox import without setAccessLevel and opens the copy4000 catalog (Drive configured: %s)',async(driveConfigured)=>{
  ...
  const catalog={url,sourceId:'mangacopy',id:'mangacopy:sample',title:'Sample',observedAt:1,complete:true,note:'',groups:[],entries:[]};
  vi.mocked(readWebsiteCatalog).mockResolvedValue(catalog);
