...
import {commitCopies,putBlob,cacheSize} from '../src/library/store';
...
beforeAll(()=>{stubAuthLocks();const data=new Map<string,string>();vi.stubGlobal('localStorage',{getItem:(key:string)=>data.get(key)??null,setItem:(key:string,value:string)=>data.set(key,value),removeItem:(key:string)=>data.delete(key)});vi.stubGlobal('createImageBitmap',vi.fn(async()=>({width:10,height:20,close(){}})));});
it('reuses originals and same-account translations across different imported files, even at a full budget',async()=>{
 ...
 await saveSession(testSession({token:'test',user:{id:'cache-user',name:'reader',role:'reader'},apiOrigin:API_ORIGIN}));
 ...
 await saveSession(testSession({token:'other',user:{id:'other-user',name:'reader',role:'reader'},apiOrigin:API_ORIGIN}));
