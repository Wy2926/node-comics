...
export class Api {
  ...
  async uploadOriginal(plan:UploadPlan,blob:Blob) {
    ...
    if(url.username||url.password||url.protocol!=='https:'&&!(local&&url.protocol==='http:')||plan.method!=='PUT'||plan.authorization_required&&url.origin!==origin)throw new ApiError(msg("原图上传授权地址无效。"),'INVALID_UPLOAD_PLAN');
    if(Date.parse(plan.expires_at)<=Date.now())throw new ApiError(msg("上传授权已到期，请刷新后继续。"),'UPLOAD_URL_EXPIRED');
    return this.pool.run(async()=>{
      ...
      catch {throw new ApiError(msg("原图上传暂未完成，请检查网络和对象存储跨域配置。"),'UPLOAD_FAILED');}
      if(!response.ok)throw new ApiError(msg("原图上传未完成，将刷新上传授权后重试。"),'UPLOAD_FAILED',response.status);
      await this.assertAuthorized();
    });
  }
