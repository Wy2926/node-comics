...
export interface Job { result?: {key:string;recoverable:boolean}; id: string; input_asset_id: string | null; requested_asset_id?: string | null; output_asset_id: string | null; mode: Mode; target_language: string; status: JobStatus; phase: string; error?: { code: string; message: string }; quota_pages: number; quota_kind?:QuotaKind; quota_period_id?:string|null; created_at: string; completed_at?: string; version: number; cache_hit: boolean; reused?:boolean; quality_flags?: string[]; result_available?: boolean; result_expired?: boolean; settlement?: 'reserved'|'settled'|'released'|'free'|'included'; cancel_requested?:boolean; file_hash?:string|null; page_index?:number|null; image_sha256?:string; priority?:'realtime'|'preload'; priority_rank?:number; updated_at?:string; }
...
export interface FilePageMatch extends FilePageSource { asset: ImageAsset | null; translations: TranslationSnapshot[]; display_translations?: TranslationSnapshot[]; }
export interface TranslationImage {sha256:string;byte_size:number;content_type:string;}
export type TranslationInput = ({image:TranslationImage;mode:Mode;target_language:string}|{retry_of:string}|{regenerate_of:string;acknowledge_unknown_cost?:boolean}) & {priority?:'current'|'prefetch'};
export interface TranslationResult {kind:'translated'|'no_text'|'partial';asset_id?:string|null;width?:number;height?:number;download_url?:string|null;download_expires_at?:string|null;authorization_required?:boolean;quality_flags?:string[];}
export interface TranslationSnapshot {id:string;state:'needs_input'|'queued'|'running'|'succeeded'|'failed'|'needs_attention';mode:Mode;target_language:string;image_sha256?:string;input_asset_id?:string|null;input_expires_at?:string|null;created_at?:string;updated_at?:string;result?:TranslationResult|null;error?:{code:string;message:string}|null;}
...
export interface FeedbackRecord {id:string;translation_id:string;output_asset_id:string;issues:FeedbackIssue[];comment:string;status:'received'|'reviewing'|'resolved';created_at:string;updated_at:string;}
