...
export interface Job { id: string; input_asset_id: string; output_asset_id: string | null; mode: Mode; target_language: string; status: JobStatus; phase: string; error?: { code: string; message: string }; cost: number; created_at: string; completed_at?: string; version: number; cache_hit: boolean; quality_flags?: string[]; }
...
export interface Settings { apiBase: string; language: string; translationMode: Mode; direction: 'ltr' | 'rtl'; layout: 'continuous' | 'single'; fit: 'width' | 'window'; autoTranslate: boolean; autoLimit: number; cacheLimitMb: number; }
...
export const defaults: Settings = { apiBase: 'http://127.0.0.1:18088', language: 'zh-Hans', translationMode: 'classic', direction: 'rtl', layout: 'continuous', fit: 'width', autoTranslate: false, autoLimit: 10, cacheLimitMb: 512 };
