...
export function DownloadManagement({notify,onOpen}:{notify:(message:string)=>void;onOpen:(id:string)=>void}){
 ...
 return <section className="settings-card"><div className="nc-section-heading"><h2>{msg("网站下载资料")}</h2><button className="text-link" onClick={()=>setExpanded(v=>!v)}>{expanded?msg("收起"):msg("管理下载")} · {tasks.length}</button></div>{expanded&&tasks.slice(0,100).map(task=><div className="setting-row" key={task.id}><div><b>{task.title}</b><p>{labels[task.status]} · {task.completed} / {task.total??'?'} {msg("页")}</p>{task.error&&<p role="status">{task.error}</p>}</div><div className="nc-inline"><button className="text-link" onClick={()=>onOpen(task.entryId)}>{msg("阅读")}</button>{['running','queued'].includes(task.status)?<button className="button secondary small" onClick={()=>void pauseDownloads([task.entryId]).catch(e=>notify(e.message))}>{msg("暂停")}</button>:task.status!=='complete'?<button className="button secondary small" onClick={()=>void grantDownloads([task.entryId],task.origins).catch(e=>notify(e.message))}>{msg("授权并继续")}</button>:null}<button className="text-link" onClick={()=>setRemove(task.entryId)}>{msg("删除下载资料")}</button></div></div>)}{remove&&<Modal title={msg("删除已下载的原图？")} subtitle={msg("保留书架来源、阅读位置和译图。以后阅读时按需从网站获取。")} onClose={()=>setRemove(undefined)}><button className="button danger" onClick={()=>{const id=remove;setRemove(undefined);void deleteDownloads(id).catch(e=>notify(e.message));}}>{msg("确认删除下载资料")}</button></Modal>}</section>;
}
