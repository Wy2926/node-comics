...
export function AccountUsage({api,onLogin,onEntitlements}:{api:Api;onLogin:()=>void;onEntitlements:(value:Entitlements)=>void}) {
  ...
  return <div>
    <div className="nc-section-heading"><div><h2><Icon name="chart"/> 用量统计</h2><p className="nc-muted">查看所选时段的翻译交付与使用情况。</p></div>
    ...
    {!api.token?<div className="nc-empty"><Icon name="book" size={36}/><h2>登录后查看用量</h2><p>常规与重绘分别计量，成功交付后扣减有限额度。</p><button className="button primary" onClick={onLogin}>登录账户</button></div>:<>
      ...
      {!data&&!error?<p className="nc-loading">正在汇总用量…</p>:data&&<>
        <div className="nc-stat-grid">
          <div className="nc-stat"><span><Icon name="book" size={17}/>常规交付</span><b>{data.by_mode.classic??0}<small>页</small></b><p>其中 PLUS 权益内交付 {data.included_delivered} 页</p></div>
          <div className="nc-stat"><span><Icon name="spark" size={17}/>重绘交付</span><b>{data.by_mode.redraw??0}<small>页</small></b><p>扣减重绘额度 {data.quota_used.redraw??0} 页</p></div>
          <div className="nc-stat"><span><Icon name="check" size={17}/>未扣量交付</span><b>{data.free_delivered}<small>页</small></b><p>包括部分完成和释放预占后的补交付</p></div>
        </div>
        <div className="nc-usage-charts"><section className="nc-chart-card"><h2>每日交付页数</h2><p>{data.start_date} — {data.end_date} · {data.timezone}</p>
          ...
          {!data.delivered&&<p className="nc-chart-zero">所选时段暂无交付</p>}
