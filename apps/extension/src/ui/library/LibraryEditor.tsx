...
const labels:Record<EditorKind,string>={get work(){return msg("编辑作品");},get chapter(){return msg("编辑章节");},get publication(){return msg("编辑卷册");},get coverage(){return msg("纠正副本归属");},get inclusion(){return msg("关联收录章节");},get publicationRelations(){return msg("关联卷册");},get version(){return msg("新建内容版本");},get relation(){return msg("关联作品");}};
export function LibraryEditor({kind,id,work,library:s,copies,run,busy,onClose,onUndo}:{kind:EditorKind;id?:string;work:ComicWork;library:LibraryState;copies:ReadingCopy[];run:LibraryRun;busy:boolean;onClose:()=>void;onUndo:(undo:UndoAssignment)=>void}){
 ...
 async function save(){
  setError('');
  if(kind==='relation'&&!value){setError(msg("请选择要关联的作品。"));return;}
  let undo:UndoAssignment|undefined;
  const ok=await run('editor',msg("正在保存"),async()=>{try{await editLibrary((state,all)=>{
   const evidence={status:'user' as const,source:msg("用户管理")};
   const current=state.works.find(w=>w.id===work.id);if(!current)throw Error(msg("作品已移除，请返回书架。"));
   current.updatedAt=Date.now();
   if(kind==='work')Object.assign(current,{title:title.trim(),coverCopyId:value||undefined});
   if(kind==='chapter'){const item=state.chapters.find(c=>c.id===id);if(!item)throw Error(msg("章节已移除。"));Object.assign(item,{title:title.trim(),number:number||undefined,role:value});}
   if(kind==='publication'){const item=state.publications.find(p=>p.id===id);if(!item)throw Error(msg("卷册已移除。"));Object.assign(item,{title:title.trim(),number:number||undefined,seriesId:value||undefined,isbn:extra||undefined});}
   if(kind==='coverage'){
    const copy=all.find(c=>c.id===id);if(!copy)throw Error(msg("副本已移除。"));
   ...
   if(kind==='relation'){if(value===work.id)throw Error(msg("请选择另一部作品。"));if(state.relations.some(r=>r.fromId===work.id&&r.toId===value&&r.kind===(extra||'related')))throw Error(msg("这条关联已经存在。"));state.relations.push({id:crypto.randomUUID(),fromId:work.id,toId:value,kind:(extra||'related') as LibraryState['relations'][number]['kind'],evidence});}
  });}catch(reason){setError(reason instanceof Error?reason.message:msg("保存失败，请重试。"));throw reason;}},msg("{0}已保存", {"0": labels[kind]}));
 ...
 return <Modal title={labels[kind]} onClose={()=>{if(!busy)onClose();}}>
  ...
  {kind==='coverage'?<ImportAssignmentFields value={assignment} onChange={setAssignment} library={s}/>:kind==='inclusion'||kind==='publicationRelations'?<PublicationChoices kind={kind} title={publication?.title??''} chapters={availableChapters} publications={availablePublications} checks={checks} busy={busy} onToggle={toggle}/>:kind==='relation'?<><label className="field">{msg("作品")}<select value={value} onChange={e=>setValue(e.target.value)}><option value="">{msg("选择一部作品")}</option>{s.works.filter(w=>w.id!==work.id).map(w=><option key={w.id} value={w.id}>{w.title}</option>)}</select></label><label className="field">{msg("关系")}<select value={extra||'related'} onChange={e=>setExtra(e.target.value)}>{Object.entries(relationLabels).map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label></>:<>
   <label className="field">{msg("名称")}<input value={title} onChange={e=>setTitle(e.target.value)} maxLength={180}/></label>
   {['chapter','publication'].includes(kind)&&<label className="field">{msg("显示编号")}<input value={number} onChange={e=>setNumber(e.target.value)}/></label>}
   {kind==='chapter'&&<label className="field">{msg("内容性质")}<select value={value} onChange={e=>setValue(e.target.value)}><option value="unknown">{msg("未分类")}</option><option value="main">{msg("正文")}</option><option value="extra">{msg("番外")}</option></select></label>}
   {kind==='publication'&&<>{s.series.some(x=>x.workIds.includes(work.id))&&<label className="field">{msg("已有出版套系")}<select value={value} onChange={e=>setValue(e.target.value)}><option value="">{msg("未指定")}</option>{s.series.filter(x=>x.workIds.includes(work.id)).map(x=><option key={x.id} value={x.id}>{x.title}</option>)}</select></label>}<label className="field">{msg("ISBN（选填）")}<input value={extra} onChange={e=>setExtra(e.target.value)}/></label></>}
   {kind==='work'&&<label className="field">{msg("封面")}<select value={value} onChange={e=>setValue(e.target.value)}><option value="">{msg("自动选择")}</option>{[...copies].sort((a,b)=>a.title.localeCompare(b.title,'zh-CN',{numeric:true})||a.createdAt-b.createdAt||a.id.localeCompare(b.id)).map(c=><option key={c.id} value={c.id}>{c.title}</option>)}</select></label>}
   {kind==='version'&&<><div className="nc-form-grid"><label className="field">{msg("语言（选填）")}<input value={value} onChange={e=>setValue(e.target.value)}/></label><label className="field">{msg("译者／制作者（选填）")}<input value={extra} onChange={e=>setExtra(e.target.value)}/></label></div><p className="nc-muted">{msg("选择要归入此版本的副本，也可以稍后在副本详情中选择。")}</p><div className="nc-choice-grid">{copies.map(c=><button className="nc-choice-card" key={c.id} aria-pressed={checks.has(c.id)} onClick={()=>toggle(c.id)}><span>{c.title}</span><b>{checks.has(c.id)?msg("已选择"):msg("选择")}</b></button>)}</div></>}
  </>}
  </div>
  <div className="nc-modal-footer"><button className="button secondary" disabled={busy} onClick={onClose}>{noOptions?msg("返回卷册详情"):msg("取消")}</button><button className="button primary" disabled={busy||noOptions||(needsTitle&&!title.trim())} onClick={()=>void save()}>{busy?msg("保存中…"):msg("保存修改")}</button></div>
 </Modal>;
}
