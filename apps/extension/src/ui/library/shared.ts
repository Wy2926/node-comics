...
export const copyComplete = (copy:ReadingCopy)=>copy.discoveryComplete&&copy.pages.length>0&&savedPages(copy)===copy.pages.length;
...
export const copySummary = (copy:ReadingCopy)=>copyComplete(copy)?`${copy.retention==='offline'?'离线可读':'已缓存'} · ${copy.pages.length} 页`:`已保存 ${savedPages(copy)} / ${copy.knownTotal??(copy.discoveryComplete?copy.pages.length:'待确认')} 页`;
