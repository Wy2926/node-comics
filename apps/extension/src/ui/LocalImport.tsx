...
import {LocalImportQueue,importSummary,selectableImport,type ImportStatus} from '../comics/application/import-queue';
...
const labels:Record<ImportStatus,string>={get checking(){return msg("检查重复中");},get ready(){return msg("待导入");},get duplicate(){return msg("已存在 · 跳过");},get queued(){return msg("等待导入");},get importing(){return msg("导入中");},get created(){return msg("已导入");},get failed(){return msg("需要处理");},get cancelled(){return msg("已取消");}};
const size=(bytes:number)=>bytes<1024*1024?`${Math.max(1,Math.ceil(bytes/1024))} KB`:`${(bytes/1024/1024).toFixed(1)} MB`;
interface Props {queue:LocalImportQueue;expanded:boolean;onExpand:()=>void;onCollapse:()=>void;onAdd:()=>void;onOpen:(id:string)=>void;library:LibraryViewModel;limitMb:number;assignmentPreset?:ImportAssignment;presetKey?:number;}
export function LocalImport({queue,expanded,onExpand,onCollapse,onAdd,onOpen,library,limitMb,assignmentPreset,presetKey}:Props){
 ...
 useEffect(()=>{if(!items.length){setAssignment(assignmentPreset);setSeparate(false);setFilter('all');}},[items.length,assignmentPreset]);
 useEffect(()=>{if(!running&&phase!=='paused'){setAssignment(assignmentPreset);setSeparate(false);}},[presetKey]);
 ...
 const settled=items.filter(item=>['created','duplicate','failed','cancelled'].includes(item.status));
 ...
 const visible=items.filter(item=>filter==='all'||filter==='pending'&&['ready','checking','queued','importing'].includes(item.status)||filter==='duplicate'&&item.status==='duplicate'||filter==='failed'&&item.status==='failed');
 return <Modal title={msg("导入本地漫画")} subtitle={msg("保存完整源文件并建立页面目录；多张图片组成一份图集。")} onClose={onCollapse} closeLabel={msg("收起导入面板")} className="nc-local-import-modal">
  <div className="nc-local-import">
   ...
   <section className="nc-import-overview" aria-label={msg("导入概况")}>
    ...
    <p role="status" aria-live="polite">{checking?msg("仅检查文件签名，保存过程中计算摘要并共享相同源文件。"):locked?msg("{0} / {1} 项已处理 · {2} 项等待导入", {"0": settled.length, "1": total, "2": queued.length}):summary}</p>
    ...
    {visible.map(item=>{const same=items.find(other=>other.id!==item.id&&!!item.key&&other.key===item.key);const canSelect=selectableImport(item)&&!locked;
     return <article key={item.id} data-import-id={item.id} className={'nc-import-item '+item.status} aria-label={item.title}>
      <div className="nc-import-item-main">
       {canSelect?<input type="checkbox" aria-label={msg("导入 {0}", {"0": item.title})} checked={item.selected} disabled={checking} onChange={e=>queue.select(item.id,e.target.checked)}/>:<span className="nc-import-item-icon">{['checking','importing'].includes(item.status)?<span className="spinner"/>:<Icon name={['created','duplicate'].includes(item.status)?'check':item.status==='failed'?'info':'file'} size={18}/>}</span>}
       <div className="nc-import-item-body"><div className="nc-import-item-title"><strong>{item.files.length>1?msg("{0} 等 {1} 张图片", {"0": item.title, "1": item.files.length}):item.title}</strong><span className={'nc-status-badge '+(['created','duplicate'].includes(item.status)?'complete':item.status==='failed'?'failed':item.status==='importing'?'active':'')}>{labels[item.status]}</span></div><p className="nc-import-item-message" role={item.status==='failed'?'alert':undefined}>{item.message}</p>
        ...
        <div className="nc-import-item-meta"><small>{size(item.bytes)}{item.files.length>1?msg(" · {0} 页图片", {"0": item.files.length}):' · '+item.files[0].name.split('.').at(-1)?.toUpperCase()}</small>
         <div className="nc-import-item-actions">{item.copyId&&['duplicate','created'].includes(item.status)&&<button className="text-link" onClick={()=>{onCollapse();onOpen(item.copyId!);}}>{item.status==='duplicate'?msg("打开已有漫画"):msg("开始阅读")}</button>}
