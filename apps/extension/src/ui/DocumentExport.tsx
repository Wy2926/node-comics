...
import type {Document} from '../comics/domain';
...
export interface DocumentExportProps {document:Document;api?:Api;userId?:string;settings:Pick<Settings,'language'|'translationMode'>;onClose():void}
export function DocumentExport({document:doc,api,userId,settings,onClose}:DocumentExportProps){
  ...
  return <Modal title={msg("导出漫画")} subtitle={doc.title} onClose={()=>{active.current?.abort();onClose();}}>
    <div className="nc-import-assignment">
      <p>{msg("导出会主动读取所选文档的全部页面，可能从云盘或来源网站下载图片。已有译图导出不创建翻译任务，缺少译图的页面保留原图。")}</p>
