...
export function FeedbackInbox({api}:{api:Api}){
  ...
  return <section className="settings-card nc-feedback-inbox"><div className="nc-section-heading"><h2><Icon name="message"/>我的反馈</h2><button className="button secondary" onClick={()=>setRefresh(v=>v+1)}><Icon name="refresh" size={17}/>刷新</button></div>{error?<p role="alert" className="inline-error">{error}</p>:!data?<p className="nc-muted">正在读取反馈…</p>:!data.items.length?<p className="nc-empty-copy">暂无反馈</p>:<>{data.items.map(row=><article className="nc-feedback-record" key={row.id}><div><b>{row.issues.map(i=>issueLabels[i]).join(' · ')||'补充说明'}</b><p className="nc-preserve-lines">{row.comment||'未填写补充说明'}</p><span className="nc-muted">{new Date(row.created_at).toLocaleString()} · 任务 {row.job_id.slice(0,8)}</span></div><span className="nc-state">{feedbackLabels[row.status]}</span></article>)}<div className="nc-pagination"><span>共 {data.total} 条反馈</span><button className="button secondary" disabled={!offset} onClick={()=>setOffset(v=>Math.max(0,v-20))}>上一页</button><button className="button secondary" disabled={data.next_offset==null} onClick={()=>setOffset(data.next_offset!)}>下一页</button></div></>}</section>;
}
