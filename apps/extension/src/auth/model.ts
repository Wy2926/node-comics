...
export function secureIdentityUrl(value:string){
  ...
  if(url.username||url.password||url.protocol!=='https:'&&!(['127.0.0.1','localhost'].includes(url.hostname)&&url.protocol==='http:'))throw Error(msg("身份服务必须使用 HTTPS。"));
  return url;
}
export function tokenLifetime(seconds:unknown,issuedAt=Date.now()){
  if(typeof seconds!=='number'||!Number.isFinite(seconds)||seconds<=0||seconds>31536000)throw Error(msg("身份服务未返回有效的令牌有效期，请重新登录。"));
