...
async function exchange(callback:string,pending:Pending):Promise<Session> {
  ...
  if(!response.ok){
    ...
    if(failure?.error==='invalid_request'&&typeof failure.error_description==='string'&&/origin .+ not allowed for client/.test(failure.error_description))throw Error('登录来源未获身份服务允许，请管理员在 Logto 应用的 Allowed CORS origins 中登记当前网页或插件来源。');
    if(failure?.error==='invalid_grant')throw Error('登录授权码已失效或校验失败，请重新登录。');
    throw Error(`登录授权码交换失败（HTTP ${response.status}），请重新登录；若仍失败，请联系管理员检查身份服务配置。`);
  }
  const tokens=await response.json();if(typeof tokens.access_token!=='string'||!tokens.access_token||tokens.token_type?.toLowerCase()!=='bearer')throw Error('身份服务未返回有效的访问令牌。');
  if(typeof tokens.refresh_token!=='string'||!tokens.refresh_token)throw Error('身份服务未授予自动续期权限，请重新登录并授权离线访问；若仍失败，请联系管理员检查应用配置。');
...
export async function startOidc(config:AuthConfig,apiBase:string):Promise<Session|null> {
  ...
  if(extension){const permissions={origins:[...new Set([authorization.origin+'/*',token.origin+'/*'])]};const granted=await chrome.permissions.contains(permissions)||await chrome.permissions.request(permissions);if(!granted)throw Error('需要身份服务的访问权限才能完成登录。');}
