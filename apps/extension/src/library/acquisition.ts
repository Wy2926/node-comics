 ...
 async run(limitMb:number){
  ...
  await navigator.locks.request('nc-library-acquisition',{ifAvailable:true},async lock=>{
   ...
   while(!this.stopped){
    ...
    if(!task)break;
    try{await this.acquire(task.copyId,limitMb);}catch(e){await editLibrary(s=>{const t=s.tasks.find(t=>t.id===task.id);if(t&&t.status!=='paused'){t.status=this.stopped||e instanceof ImagePermissionsRequired?'paused':'failed';t.error=e instanceof Error?e.message:'采集失败';t.updatedAt=Date.now();}});}
   }
  });
 }
 ...
 private async acquire(copyId:string,limitMb:number){
  ...
  const update=async(manifest:PageManifest)=>{
   ...
   await editLibrary((s,copies)=>{
    ...
    if(c.discoveryComplete&&manifest.knownTotal&&c.pages.length!==manifest.knownTotal||manifest.discoveryComplete&&c.pages.length>manifest.items.length)throw Error('来源页数发生修订，请保留当前副本并重新导入修订版。');
    // A restarted source can expose a shorter prefix; retain saved suffix pages and identities.
    const pages=[...c.pages];
    for(const [n,item] of manifest.items.entries())pages[n]={...pages[n]??emptyPage('第 '+(n+1)+' 页',item.width,item.height),sourceUrl:item.url};
  ...
  const download=async()=>{
   ...
   while(true){
    ...
    }catch(e){await active();const message=e instanceof Error?e.message:'原图获取失败';await this.patch(copyId,page.id,{fetchError:message});if(message.includes('空间预算'))throw e;failures++;}
   }
  };
