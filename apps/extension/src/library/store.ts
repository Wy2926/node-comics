...
export async function clearTranslations(){await clearLocalImages(undefined,'translations');}
/** Clear references and unshared bytes atomically, preserving other copies and reader positions. */
export async function clearLocalImages(copyIds:string[]|undefined,kind:'originals'|'translations'|'all'){
 const database=await db();await new Promise<void>((resolve,reject)=>{
  const tx=database.transaction(['copies','blobs','library'],'readwrite'),records=tx.objectStore('copies'),blobs=tx.objectStore('blobs'),meta=tx.objectStore('library');
  const library=meta.get('library'),cachedKeys=blobs.getAllKeys(),request=records.getAll();
  request.onsuccess=()=>{
   const copies=request.result as ReadingCopy[],selected=new Set(copyIds??copies.map(c=>c.id)),candidates=new Set<string>();
   for(const copy of copies){if(!selected.has(copy.id))continue;
    const position=readPosition(copy.id,copy.manifestRevision);if(position)Object.assign(copy,position);
    copy.manifestRevision++;
...
export async function removeWork(workId:string){await removeWorks([workId]);}
export async function removeWorks(workIds:string[]){
