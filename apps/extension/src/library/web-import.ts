...
export function insertionBlocked(copy:ReadingCopy,library:LibraryState):string|undefined{
 if(library.tasks.some(task=>task.copyId===copy.id&&['queued','running'].includes(task.status)))return msg("请先在采集中心暂停此副本，再插入图片。");
 if(copy.sourceEntryId&&!copy.discoveryComplete)return msg("此副本的来源页序尚未完整，请先补齐图片清单。");
}
  ...
  for(const item of manifest.items){
   ...
   try{
    ...
    if(!await getBlob(key)){
     ...
     if(await cacheSize()+blob.size>limitMb*1024*1024)throw Error(msg("本地空间预算不足，请提高上限或清理副本后重试。"));
     await putBlob(key,blob);
    }
    page.blobKey=key;
   }catch(e){page.fetchError=(e as Error).message;}
   progress(msg("正在获取原图 {0} / {1}…", {"0": copy.pages.length, "1": manifest.items.length}));
  }
...
export async function insertWebCopy(incoming:ReadingCopy,destination:Extract<WebDestination,{mode:'insert'}>){
 const result=await editLibrary((library,copies)=>{
  ...
  if(!copy)throw Error(msg("目标副本已移除，请重新选择。"));
  if(!incoming.pages.length)throw Error(msg("没有可插入的图片。"));
  ...
  if(copy.manifestRevision!==destination.revision)throw Error(msg("目标副本的页序已改变，请重新选择插入位置。"));
  ...
  if(destination.position.kind==='after'){
   ...
   if(!at)throw Error(msg("插入位置已移除，请重新选择。"));
  }
  ...
  if(task){task.total=copy.pages.length;task.completed=copy.pages.filter(page=>page.blobKey).length;task.status=task.completed===task.total?'complete':'paused';task.error=task.status==='paused'?msg("有原图待获取，可在采集中心继续。"):undefined;task.updatedAt=copy.updatedAt;}
