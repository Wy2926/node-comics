...
export function attachCopy(state:LibraryState,copy:ReadingCopy,assignment:ImportAssignment){
 ...
 if(assignment.workId&&!work)throw Error('目标作品已移除，请重新选择。');
 ...
 if(assignment.kind==='chapter'||assignment.kind==='extra'){
   ...
   if(assignment.targetId&&!chapter)throw Error('所选章节不属于目标作品。');
   if(!chapter){chapter={id:crypto.randomUUID(),workId:work.id,title:copy.title,number:assignment.number,numbering:'用户导入序列',order:state.chapters.filter(c=>c.workId===work.id).length,role:assignment.kind==='extra'?'extra':'unknown',evidence};state.chapters.push(chapter);}
   ...
   if(assignment.targetId&&!book)throw Error('所选卷册不属于目标作品。');
   if(assignment.seriesId&&!state.series.some(s=>s.id===assignment.seriesId&&s.workIds.includes(work.id)))throw Error('出版套系不属于目标作品。');
...
export function selectRange<T extends {id:string}>(items:T[],first:string,last:string):string[]{
 ...
 if(a<0||b<0)throw Error('范围端点已不在当前目录中。');
 return items.slice(Math.min(a,b),Math.max(a,b)+1).map(i=>i.id);
}
export function validateLibrary(s:LibraryState){
 const unique=(ids:string[])=>new Set(ids).size===ids.length;
 for(const collection of [s.works,s.chapters,s.versions,s.series,s.publications,s.inclusions,s.publicationRelations,s.coverage,s.relations,s.catalogs,s.tasks])if(!unique(collection.map(x=>x.id)))throw Error('对象身份重复。');
 const work=(id:string)=>s.works.some(w=>w.id===id),book=(id:string)=>s.publications.some(p=>p.id===id);
 if(s.series.some(e=>!e.workIds.length||e.workIds.some(id=>!work(id)))||s.versions.some(v=>!work(v.workId)||v.chapterId&&!s.chapters.some(c=>c.id===v.chapterId&&c.workId===v.workId))||s.relations.some(r=>!work(r.fromId)||!work(r.toId)||r.fromId===r.toId))throw Error('版本或作品关联无效。');
 if(s.chapters.some(c=>!work(c.workId))||s.coverage.some(c=>!work(c.workId))||s.publications.some(p=>p.workIds.some(id=>!work(id))||p.seriesId&&!s.series.some(e=>e.id===p.seriesId)))throw Error('关联对象已不存在。');
 for(const r of s.publicationRelations){if(!book(r.fromId)||!book(r.toId))throw Error('关联卷册不存在。');const visited=new Set<string>();const reach=(id:string):boolean=>id===r.fromId||!visited.has(id)&&(visited.add(id),s.publicationRelations.filter(x=>x.fromId===id).some(x=>reach(x.toId)));if(reach(r.toId))throw Error('出版物收录／再版关系不能形成循环。');}
 for(const i of s.inclusions){if(!book(i.publicationId)||!(i.target.kind==='work'?work(i.target.id):s.chapters.some(c=>c.id===i.target.id)))throw Error('收录对象不存在。');}
 for(const c of s.coverage){if(c.target.kind==='chapter'&&!s.chapters.some(x=>x.id===c.target.id&&x.workId===c.workId)||c.target.kind==='publication'&&!s.publications.some(p=>p.id===c.target.id&&p.workIds.includes(c.workId))||['work','unclassified'].includes(c.target.kind)&&c.target.id!==c.workId)throw Error('副本归属无效。');}
}
