...
export function driveFormat(name: string, mimeType: string): 'cbz' | 'image' {
  if (mimeType.startsWith('application/vnd.google-apps.')) throw new DriveError('unsupported-format', '在线文档、文件夹和快捷方式暂不支持云端直接阅读。');
  if (/\.(zip|cbz)$/i.test(name)) return 'cbz';
  if (/\.(png|jpe?g|webp|gif|avif)$/i.test(name) && mimeType.startsWith('image/')) return 'image';
  throw new DriveError('unsupported-format', 'Google Drive 首轮仅开放 CBZ/ZIP 和独立图片；PDF、MOBI、RAR 请下载后从本地导入。');
}
