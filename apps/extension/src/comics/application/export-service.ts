...
export async function exportDocument(documentId:string,options:ExportOptions,context:ExportContext):Promise<ExportResult>{
  const {api,userId,signal}=context;
  const account=api&&userId?{origin:api.base,userId}:undefined;
  const plan=await planExport(documentId,options,account,signal);
  const assertCurrent=async()=>{
    if(options.images==='translation'&&(!api?.isCurrent()||context.isCurrent?.()===false))throw Error('账户或服务已切换，本次导出已停止。');
    const doc=await catalog.get('documents',documentId);
    if(!doc||doc.revisionId!==plan.revisionId||doc.generation!==plan.generation)throw Error('文档版本已变化，本次导出已停止。');
  };
...
export async function originalFileInfo(documentId:string):Promise<{name:string;size?:number}|undefined>{
  const document=await catalog.get('documents',documentId);if(!document)return;
  const revision=await catalog.get('revisions',document.revisionId);if(!revision?.containerId)return;
...
export async function exportOriginalFile(documentId:string,context:Pick<ExportContext,'signal'|'destination'|'progress'>):Promise<ExportResult>{
  context.signal.throwIfAborted();
  const doc=await catalog.get('documents',documentId);if(!doc)throw Error('文档已移除。');
  const revision=await catalog.get('revisions',doc.revisionId);if(!revision?.containerId)throw Error('此文档没有单份本地源文件，请使用页面导出。');
  const name=(await originalFileInfo(documentId))?.name??safeName(doc.title);
  const source=await openContainer(revision.containerId);
