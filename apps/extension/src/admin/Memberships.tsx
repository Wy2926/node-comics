...
export function MembershipControls({api,users,onChanged}:{api:Api;users:AdminUser[];onChanged:()=>Promise<void>}) {
  ...
  return <>
    <section className="settings-card"><h3>用户与会员权益</h3>{users.map(user=><div className="setting-row" key={user.id}><div><b>{user.name} · {user.entitlements.plan==='plus'?'PLUS':'普通用户'} · {user.role}</b><p>常规：{user.entitlements.modes.classic.unlimited?'不限量':`${user.entitlements.modes.classic.quota?.available??0} 页可用`} · 重绘：{user.entitlements.modes.redraw.quota?.available??0} 页可用 · 并发 {user.entitlements.concurrency}</p></div><button className="button secondary small" onClick={()=>{setTarget(user.id);changed();setNote('');}}>管理会员与额度</button></div>)}</section>
