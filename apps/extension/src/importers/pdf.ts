...
export async function openPdf(file:File,originalHash:string) {
  ...
  try {
    ...
    if(!pdf.numPages||pdf.numPages>MAX_PAGES)throw Error(msg("PDF 单卷最多支持 1500 页。"));
    // Rendering changes may change upload bytes. Keep old PDF identities apart.
    const fileHash=await pdfFileHash(originalHash);
    return {fileHash,total:pdf.numPages,close:()=>task.destroy(),warnings:[msg("PDF 按页转换为 PNG；原 PDF 保留在本机，未上传。")],pages:(async function*():AsyncGenerator<ComicPage> {
      let expanded=0;
      for(let pageIndex=0;pageIndex<pdf.numPages;pageIndex++) {
        const page=await pdf.getPage(pageIndex+1);
        const base=page.getViewport({scale:1});
        if(!Number.isFinite(base.width*base.height)||base.width<=0||base.height<=0)throw Error(msg("PDF 第 {0} 页尺寸无效。", {"0": pageIndex+1}));
        ...
        try {
          ...
          const blob=await new Promise<Blob>((resolve,reject)=>canvas.toBlob(value=>value?resolve(value):reject(Error(msg("PDF 页面转换失败。"))),'image/png'));
          expanded+=blob.size;
          if(blob.size>MAX_PAGE||expanded>MAX_EXPANDED)throw Error(msg("PDF 转换后超过单页 32 MB 或整卷 1024 MB 限制，请拆分文件。"));
          yield {name:msg("第 {0} 页", {"0": String(pageIndex+1).padStart(3,'0')}),pageIndex,blob,width:canvas.width,height:canvas.height};
        } finally {clearTimeout(timer);canvas.width=canvas.height=1;page.cleanup();}
      }
    })()};
  } catch(error) {
    await task.destroy();
    if((error as Error).name==='PasswordException')throw Error(msg("PDF 需要密码，请先在本机解密后导入。"));
    throw error;
  }
}
