 ...
 */
export interface MobiPage { id: string; name: string; blob: Blob; width?: number; height?: number }
export interface MobiImport { title: string; pages: MobiPage[]; warnings: string[] }
const MAX_FILE = 512 * 1024 * 1024;
const MAX_TEXT = 16 * 1024 * 1024;
const MAX_PAGES = 1500;
const decoder = new TextDecoder('utf-8');
function fail(message: string): never { throw new Error(message); }
function u16(b: Uint8Array, n: number) { if (n + 2 > b.length) fail('MOBI 文件头不完整。'); return new DataView(b.buffer, b.byteOffset, b.byteLength).getUint16(n); }
function u32(b: Uint8Array, n: number) { if (n + 4 > b.length) fail('MOBI 记录越界或已损坏。'); return new DataView(b.buffer, b.byteOffset, b.byteLength).getUint32(n); }
...
function withoutTrailers(input: Uint8Array, flags: number): Uint8Array {
  ...
  for (let bits = flags >>> 1; bits; bits >>>= 1) {
    ...
    if (!found || size <= 0 || size > end) fail('MOBI 文本附加记录无效。');
    end -= size;
  }
  if (flags & 1) {
    if (!end) fail('MOBI 文本附加记录缺失。');
    end -= (input[end - 1] & 3) + 1;
  }
  if (end < 0) fail('MOBI 文本附加记录越界。');
...
export async function importMobi(file: File, onProgress?: (done: number, total: number) => void): Promise<MobiImport> {
  ...
  if (!count || 78 + count * 8 > file.size) fail('MOBI 记录表已损坏。');
  ...
  if (offsets[0] < 78 + count * 8 || offsets.some((v, i) => i > 0 && v <= offsets[i - 1])) fail('MOBI 记录表范围或顺序无效。');
  const record = async (index: number, max = MAX_TEXT) => {
    if (index < 0 || index >= count || offsets[index + 1] - offsets[index] > max) fail('MOBI 记录超出安全限制。');
  ...
  if (u16(first, 12) !== 0) fail('此 MOBI 受 DRM 加密保护。请使用可直接读取的本地图片。');
  const version = u32(first, 36), compression = u16(first, 0), textRecords = u16(first, 8);
  if (version > 7) fail('暂不支持独立 KF8/AZW3，请导出为普通 MOBI 或图片后导入。');
  if (![1, 2].includes(compression)) fail('暂不支持此 MOBI 的 HUFF/CDIC 压缩，请导出为普通 MOBI 或图片。');
  ...
  if (!refs.length) fail('未找到漫画正文图片引用。暂不以资源排列猜测阅读顺序。');
  ...
  for (let ordinal = 0; ordinal < refs.length; ordinal++) {
    ...
    if (index < firstImage || index >= count) fail(`第 ${ordinal + 1} 页的图片记录不存在。`);
    const length = offsets[index + 1] - offsets[index];
    if (length > 32 * 1024 * 1024) fail(`第 ${ordinal + 1} 页超过 32 MB，请单独处理。`);
    ...
    if (!info) fail(`第 ${ordinal + 1} 页不是支持的 PNG、JPEG、GIF 或 WebP 图片。`);
    if (info.width && info.height && (info.width * info.height > 40_000_000 || Math.max(info.width, info.height) > 30000)) fail(`第 ${ordinal + 1} 页尺寸超出阅读器安全限制。`);
    pages.push({ id: `mobi-${ordinal + 1}-${index}`, name: `第 ${String(ordinal + 1).padStart(3, '0')} 页`,
  ...
  return { title: file.name.replace(/\.mobi$/i, ''), pages, warnings };
}
