...
export async function rarContents(data:ArrayBuffer,wasmBinary:ArrayBuffer) {
  ...
  if(list.arcHeader.flags.volume)throw Error(msg("暂不支持 RAR 分卷，请合并为单个压缩包后导入。"));
  if(list.arcHeader.flags.headerEncrypted)throw Error(msg("暂不支持加密 RAR，请在本机解密后导入。"));
  const entries=[];
  ...
  io.write=(fd,buf,size)=>{
    ...
    if(size<0||pageWritten>MAX_PAGE||totalWritten>256*MiB)throw Error(msg("RAR 实际展开大小超过安全限制。"));
  ...
  return {total:images.length,pages:(function*():Generator<ComicPage> {
    while(true) {
      ...
      if(!extraction||extraction.byteLength!==fileHeader.unpSize)throw Error(msg("RAR 图片数据不完整。"));
