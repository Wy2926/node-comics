...
export interface ComicPage { name: string; pageIndex: number; blob: Blob; width?: number; height?: number }
export interface ComicImport {
  title: string; fileHash: string; format: string; total: number; warnings: string[];
  pages: AsyncIterable<ComicPage> | Iterable<ComicPage>;
...
export function validateEntries<T extends {name:string; size:number; encrypted?:boolean}>(entries:T[], limit=MAX_EXPANDED) {
  ...
  for(const entry of entries) {
    if(entry.encrypted)throw Error('暂不支持加密压缩包，请先在本机解密后导入。');
    if(!Number.isSafeInteger(entry.size)||entry.size<0||entry.size>MAX_PAGE)throw Error('压缩包单个文件不能超过 32 MB。');
    expanded+=entry.size;
    if(expanded>limit)throw Error(`压缩包展开大小超过 ${limit/MiB} MB，请拆分为较小章节。`);
  }
  const images=entries.filter(entry=>comicImage(entry.name)).sort(comparePaths);
  if(!images.length)throw Error('压缩包内没有 PNG、JPEG、WebP 或 GIF 漫画图片。');
  if(images.length>MAX_PAGES)throw Error('单卷最多支持 1500 页。');
  if(new Set(images.map(e=>e.name)).size!==images.length)throw Error('压缩包中存在同路径重名图片，请整理后重新导入。');
  return images;
}
