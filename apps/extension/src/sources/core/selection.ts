...
import type { PageManifest, SourceItem } from '../contracts/source';
...
export function refreshChoices(previous: ImageChoice[], manifest: PageManifest): ImageChoice[] {
  ...
  const kept = previous.flatMap((old) => {
    const item = remaining.get(old.id);
    if (!item || item.url !== old.url) return [];
    remaining.delete(old.id);
    return [{ ...item, selected: old.selected }];
  });
...
export function moveChoice(choices: ImageChoice[], id: string, targetId: string): ImageChoice[] {
  const from = choices.findIndex((item) => item.id === id),
    to = choices.findIndex((item) => item.id === targetId);
...
export function selectManifest(manifest: PageManifest, itemIds: unknown): PageManifest {
  ...
  return {
    ...
    discoveryComplete: whole && manifest.discoveryComplete,
    knownTotal: whole ? manifest.knownTotal : undefined,
    note: whole
      ? manifest.note
      : msg('已选 {0} / {1} 张图片，保留所选顺序；不代表完整章节。', {
          '0': items.length,
          '1': manifest.items.length,
        }),
  };
}
