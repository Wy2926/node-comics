...
import type {PageManifest,SourceItem} from './model';
export function discoverMangaCopyDocument(doc:Document,pageUrl:string):Omit<PageManifest,'id'|'sourceTabId'|'navigationId'|'revision'>{
...
export function discoverImageDocument(doc:Document,pageUrl:string,options={id:'generic',selector:'img',singlePage:false}):Omit<PageManifest,'id'|'sourceTabId'|'navigationId'|'revision'>{
  const {id,selector,singlePage}=options,generic=!singlePage;
  ...
  return {title:doc.title.slice(0,160)||msg("未命名漫画"),url:pageUrl,adapter:id,direction:singlePage?'ltr':'rtl',discoveryComplete:(singlePage)&&items.length===1,knownTotal:(singlePage)&&items.length===1?1:undefined,note:singlePage?msg("仅当前一期／当前漫画页，不包括前后章节。"):msg("按标签页翻译规则发现已加载的网页大图；滚动原网页后可刷新补充，仅支持 HTTP(S) 原图导入。{0}", {"0": items.length>=MAX_COMIC_IMAGES?msg("已达到单次 1500 张上限。"):''}),items};
}
