...
export async function discoverCatalog(url:string):Promise<SourceCatalog>{
 ...
 try{for(let i=0;i<40;i++){await new Promise(r=>setTimeout(r,500));const t=await chrome.tabs.get(tab.id);if(t.url!==url&&t.status==='complete'&&(!t.url||!sameMangaCopyPage(t.url,url)))throw Error('来源页面跳转，请回源核实。');if(t.status!=='complete')continue;const result=await sourceMessage<{kind:string;catalog?:SourceCatalog}>({type:'NC_DISCOVER_TAB',tabId:tab.id});if(result.catalog?.complete)return result.catalog;}throw Error('目录未完整加载，请打开来源页处理后重试。');}
