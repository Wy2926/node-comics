...
- `image.ts`：独立声明图片请求头和解码函数；不与目录／页面发现绑定。当前不开放网页原位翻译。
- `page.ts`：仅提供站内导入／管理入口。详情页挂载在 `.mpage__actions`，章节页挂载在 `.rpage-floatctl` 悬浮工具栏内，避免被绝对定位的阅读区遮住；观察异步挂载和容器替换，导航后清理。目录和图片清单仍使用 HTTP，不从页面图片推断页序。
...
扩展“漫画网站”提供链接导入入口。首次操作按定义申请主站与 `*.wowpic1.store` 可选权限；主站授权后，公共运行时按 `optionalContentMatches` 注册内容脚本，在详情页和章节页显示“NodeLane Comics · 导入/管理漫画”，撤销授权后取消注册，不增加安装必需权限。不同图片域名仍需已有授权流程。目录、首次读章、重试和自动同步不创建来源采集标签页，失败不回退到开页。CDN Referer 由通用会话规则设置，只在读取期间匹配扩展自身发起到已登记精确图片 URL 的请求；共享 URL 的请求串行隔离，完成或取消后清理。Cookie 和用户令牌不上传，全部网络请求省略凭据。
