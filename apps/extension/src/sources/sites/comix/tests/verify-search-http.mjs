...
const root = process.cwd(), output = path.join(root, 'artifacts/comix/search-http');
...
await build({configFile: false, root: path.join(root, 'apps/extension'), logLevel: 'error', build: {
  ...
  lib: {entry: path.join(root, 'apps/extension/src/sources/sites/comix/network.ts'), formats: ['es'], fileName: () => 'network.mjs'},
}});
const {network} = await import(pathToFileURL(path.join(temporary, 'network.mjs')).href);
const requests = [], results = [];
for (const query of ['One Piece', 'zzzznodelanesearchzzzz']) {
  const controller = new AbortController(), timer = setTimeout(() => controller.abort(), 15000);
  const context = {signal: controller.signal, request: async target => {
    const url = new URL(target);
    assert.equal(url.origin, 'https://comix.to');
  ...
  try {
    const first = await network.search({siteId: 'comix', query}, context);
    clearTimeout(timer);
    ...
    if (query !== 'zzzznodelanesearchzzzz') assert(first.items.some(item => item.catalogId === 'comix:pvry'));
    ...
    let second;
    try { second = first.nextCursor ? await network.search({siteId: 'comix', query, cursor: first.nextCursor}, context) : undefined; }
