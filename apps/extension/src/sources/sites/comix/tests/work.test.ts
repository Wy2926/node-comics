...
const url='https://comix.to/title/fixture-work';
function work(target=url, include=true){
 const nodes:Record<string,unknown>={'#initial-data':{textContent:JSON.stringify({queries:{[JSON.stringify(['manga','detail','fixture'])]:{hid:'fixture',url:'/title/fixture-work',title:'作品名'}}})}};
...
describe('search work metadata',()=>{
 ...
 it('never promotes a chapter title to a work title',()=>expect(work('https://comix.to/title/fixture-work/1-chapter-1').status).toBe('not-ready'));
 it('rejects metadata from another work after navigation',()=>expect(work('https://comix.to/title/other-work').status).toBe('not-ready'));
});
