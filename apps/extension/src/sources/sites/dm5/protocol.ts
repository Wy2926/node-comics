...
export function imageUrls(response: string, chapterId: string, comicId: number): string[] {
  ...
  if (!Array.isArray(items) || !items.length || items.length > 2) throw Error('DM5 图片列表为空或数量已变化。');
  ...
  return items.map(item => {
    ...
    const url = new URL(absolute ? path : prefix + path + stringLiteral(suffix[1]), 'https://www.dm5.com');
    if (url.protocol !== 'https:' || !url.hostname.endsWith('.cdndm5.com') || url.username || url.password || url.port ||
        !new RegExp(`^/\\d+/${comicId}/${chapterId}/[^/]+$`).test(url.pathname) || url.hash)
