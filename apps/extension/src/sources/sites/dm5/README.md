...
支持 HTTPS `www.dm5.com`／`dm5.com` 的 `/manhua-<slug>/` 漫画详情和 `/m<chapter-id>/` 阅读页（包含 `-p<page>` 分页地址）。详情页导入完整漫画；直接从未绑定的阅读页导入时，来源资源是当前章节。仅支持源站公开可读内容；登录、购买、下架和验证页面明确报错，不打开采集标签页兜底。不提供网页原位翻译。
