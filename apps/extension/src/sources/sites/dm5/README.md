...
- 搜索复用公开页面 `/search?title=<关键词>&page=<页码>`，依据源站搜索按钮和 [search.js](https://css99tel.cdndm5.com/v202609180933/dm5/js/search.js)。读取专门主结果和 `.mh-list`，校验查询回显、作品地址及分页；游标只保留页码。该引擎会对无匹配词返回近似作品，因此结果始终是候选，不能认定同一漫画。不发送语言筛选；兼容源站默认分页链接中的 `language=1`。结果未提供语言时不声明语言。
