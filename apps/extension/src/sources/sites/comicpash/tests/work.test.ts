...
const url='https://comicpash.jp/series/fixture';
function work(target=url, include=true){
 const nodes:Record<string,unknown>={'link[rel="canonical"]':{href:'https://comicpash.jp/series/fixture'},'meta[property="og:title"]':{content:'作品名'}};
...
describe('search work metadata',()=>{
 ...
 it('never promotes a chapter title to a work title',()=>expect(work('https://comicpash.jp/episodes/fixture').status).toBe('not-ready'));
 it('rejects metadata from another work after navigation',()=>expect(work('https://comicpash.jp/series/other').status).toBe('not-ready'));
});
