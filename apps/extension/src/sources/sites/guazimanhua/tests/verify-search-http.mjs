...
const root=process.cwd(), output=path.join(root,'artifacts/guazimanhua/search-http');
...
await build({configFile:false,root:path.join(root,'apps/extension'),logLevel:'error',build:{
  outDir:temporary,emptyOutDir:false,lib:{entry:path.join(root,'apps/extension/src/sources/sites/guazimanhua/network.ts'),formats:['es'],fileName:()=> 'network.mjs'},
}});
...
const cases=[{siteId:'guazimanhua',query:'海'}, {siteId:'guazimanhua',query:'zzzznodelanesearchzzzz',empty:true}], results=[];
