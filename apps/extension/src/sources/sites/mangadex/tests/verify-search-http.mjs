...
const root=process.cwd(), output=path.join(root,'artifacts/mangadex/search-http');
...
const context={request:async target=>{
  ...
  assert(!url.searchParams.has('availableTranslatedLanguage[]'));
  assert(!url.searchParams.has('originalLanguage[]'));
...
const cases=[{siteId:'mangadex',query:'キン肉マンII世 究極の超人タッグ編',expectedId:'mangadex:af857779-a400-46ef-917a-3b3a248e2221'}, {siteId:'mangadex',query:'Love'}, {siteId:'mangadex',query:'zzzznodelanesearchzzzz',empty:true}], results=[];
for(const {empty,expectedId,...query} of cases){
  const first=await network.search(query,context);
  assert(first.items.length<=50);
  if(expectedId)assert(first.items.some(item=>item.catalogId===expectedId));
