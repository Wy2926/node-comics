...
export async function requestImagePermissions(origins:string[]){
 ...
 if(origins.length&&inExtension()&&!await chrome.permissions.request({origins:[...new Set(origins)]}))throw Error('图片域名未获授权；尚未开始下载，可再次授权。');
}
...
export class ImagePermissionsRequired extends Error {
 constructor(){super('需要授权图片域名。点击“授权并继续”后开始下载，已发现链接已保留。');}
}
