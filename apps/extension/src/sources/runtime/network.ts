...
function networkContext(signal?:AbortSignal){return {signal,async request(url:string){
...
export async function readNetworkCatalog(url:string,options:{signal?:AbortSignal;previous?:SourceCatalogSnapshot}={}){
  ...
  const snapshot=validateCatalog(await read(url,{...networkContext(signal),previous}),definitions);
...
export async function readNetworkPages(url:string,signal?:AbortSignal):Promise<PageManifest>{
  ...
  const snapshot=validatePages(await read(url,networkContext(signal)),location);
  signal?.throwIfAborted();
