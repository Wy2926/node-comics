import type {ComicElement} from './model';
...
export function comicImageRect(image:ComicElement):DOMRect|undefined{
 ...
 if(!comicSize(rect.width,rect.height)||('naturalWidth' in image?(!image.complete||image.naturalWidth<80||image.naturalHeight<80):(image.width<80||image.height<80)))return;
