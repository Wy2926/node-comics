...
export async function readMangaCopyData(doc:Document,pageUrl:string){
 ...
 if(data.length!==1)throw Error('来源图片数据不唯一，请刷新来源页面后重试。');
 ...
 if(![16,24,32].includes(rawKey.length)||iv.length!==16||!hex.length||hex.length>8*1024*1024||hex.length%32||!/^[a-f\d]+$/i.test(hex))throw Error('来源图片数据格式已变化，无法读取图片清单。');
 let decoded:unknown;
 try{
 ...
 }catch{throw Error('来源图片数据无法解码，请刷新来源页面后重试。');}
 if(!Array.isArray(decoded)||!decoded.length||decoded.length>10000)throw Error('来源图片清单不可用。');
 if(before.knownTotal&&decoded.length!==before.knownTotal)throw Error('来源图片清单与网页总页数不一致，请刷新后重试。');
 const items:SourceItem[]=decoded.map((value:unknown,order)=>{
  ...
  if(!url)throw Error('来源图片清单包含无效地址。');
 ...
 if(images.length>items.length)throw Error('来源页面与图片清单不一致，请重新发现。');
 for(const [index,img] of images.entries()){
  const url=safeImageUrl(img.getAttribute('data-src')??'',pageUrl);
  if(url&&url!==items[index].url)throw Error('来源页面的图片顺序与清单不一致，请重新发现。');
 ...
 return {...before,items,knownTotal:items.length,discoveryComplete:true,note:'已读取网页 JS 的完整有序图片清单，并核对已显示图片；原图下载进度单独记录。'};
}
