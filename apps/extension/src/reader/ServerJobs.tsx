...
export function ServerJobs({api,knownIds}:{api:Api;knownIds:string[]}){
  ...
  useEffect(()=>{if(!api.token)return;let active=true;let timer:ReturnType<typeof setTimeout>;async function fetchPage(){try{const response=await api.request<{items:Job[];total:number}>(`/v1/jobs?offset=${offset}&limit=30`);if(active&&api.isCurrent()){setJobs(response.items);setTotal(response.total);setError('');}}catch(e){if(active&&api.isCurrent())setError((e as Error).message);}if(active)timer=setTimeout(fetchPage,5000);}void fetchPage();return()=>{active=false;clearTimeout(timer);};},[api,offset]);
  if(!api.token)return null;
  const remaining=jobs.filter(j=>!knownIds.includes(j.id));
  async function download(job:Job){try{const blob=await api.image(job.output_asset_id!);if(!api.isCurrent())return;const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`AI-${job.target_language}-v${job.version}.${blob.type==='image/jpeg'?'jpg':blob.type==='image/webp'?'webp':'png'}`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}catch(e){if(api.isCurrent())setError((e as Error).message);}}
