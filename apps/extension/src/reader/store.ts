  ...
  await new Promise<void>((resolve,reject)=>{
    ...
    request.onsuccess=()=>{
      ...
      const pages=chapter.pages.map(page=>{
        ...
        return {...page,...(useExistingAsset?{assetId:existing.assetId,assetExpiresAt:existing.assetExpiresAt,ownerId:existing.ownerId,apiOrigin:existing.apiOrigin}:{}),jobs:mergeJobs(existing.jobs,page.jobs)};
      });
...
// Do not carry retired fields (including the old autoTranslate flag) into new preferences.
