...
export function ComicDirectory({directory,index,pageCount,onNavigate,children}:{directory:ReadingDirectory;index:number;pageCount:number;onNavigate:(id:string,pageId?:string)=>void;children?:ReactNode}){
 ...
 return <><div className="nc-comic-directory-heading"><h3>{directory.title}</h3></div>
  ...
  {directory.sourceUrl&&<div className="nc-directory-footer"><a href={directory.sourceUrl} target="_blank" rel="noreferrer">{msg('打开来源')}</a></div>}
 </>;
}
