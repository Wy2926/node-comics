...
export function restoreImported(chapters:Chapter[],incoming:Chapter):Chapter {
  ...
  return {...existing,pages:existing.pages.map(p=>{const restored=incomingPages.get(key(p)!)!;return {...p,blobKey:restored.blobKey,width:restored.width,height:restored.height,imageSha256:restored.imageSha256,fetchError:undefined};})};
}
