...
export function PageTranslationBar({selectedView,shownJob,onView,onFeedback,translationLabel,panel,onPanel}:{
  ...
  return <nav className="nc-reader-rail right nc-reader-controls" aria-label={msg("翻译与阅读工具")}>
    <div className="nc-page-versions" role="group" aria-label={msg("本页查看方式")} data-shown-job={shownJob?.id??'original'}>
      {(['original','classic','redraw'] as const).map(value=><button key={value} aria-label={{original:msg("原图"),classic:msg("常规翻译"),redraw:msg("AI 重绘")}[value]} title={{original:msg("查看原图"),classic:msg("查看常规译图"),redraw:msg("查看 AI 重绘")}[value]} aria-pressed={selectedView===value} onClick={()=>onView(value)}><Icon name={{original:'image',classic:'globe',redraw:'spark'}[value]}/><span>{{original:msg("原图"),classic:msg("常规"),redraw:'AI'}[value]}</span></button>)}
    </div>
    ...
    {shownJob&&<button aria-label={msg("译图有问题")} title={msg("译图有问题")} onClick={onFeedback}><Icon name="info"/><span>{msg("反馈")}</span></button>}
  </nav>;
}
