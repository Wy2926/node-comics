...
export function PageTranslationBar({page,number,mode,shownJob,languageId,ownerId,origin,note,preparing,busy,caps,onView,onTranslate,onProgress,onFeedback}:{
  page:Page;number:number;mode:Mode;shownJob?:Job;languageId:string;ownerId?:string;origin:string;note:string;preparing:boolean;busy:boolean;caps?:Capabilities;
  ...
  return <div className="nc-reader-status">
    ...
    <div className="nc-page-versions" role="group" aria-label="本页查看方式" data-shown-job={shownJob?.id??'original'}>
      <button aria-pressed={!shownJob} onClick={()=>onView('original')}>原图</button>
      {(['classic','redraw'] as const).map(value=>{
        const t=value==='classic'?classic:redraw;
        return <button key={value} aria-pressed={shownJob?.mode===value} title={`${labels[value]} · ${t.pending?taskText(t.pending):t.ready?'已就绪':t.expired?'已过期':'尚未生成'}`} onClick={()=>onView(value)}>{labels[value]}</button>;
      })}
    </div>
    {mode==='classic'&&action('classic')}
    {action('redraw')}
    {note&&<button className="nc-task-note" onClick={onProgress}>{preparing&&<TaskActivity/>}{note}</button>}
