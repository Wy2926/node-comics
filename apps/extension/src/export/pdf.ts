...
export async function pdfWriter(title: string) {
  ...
  doc.setTitle(title); doc.setCreator('Node Comics'); doc.setProducer('Node Comics / pdf-lib');
  let bytes = 0;
  return {
    async add(blob: Blob) {
      const image = await exportImage(blob, true);
      bytes += image.blob.size;
      if (bytes > 128 * 1024 * 1024) throw Error('单份 PDF 图片超过 128 MiB，请改用 CBZ 或图片 ZIP。');
    ...
    async close(manifest: string) {
      await doc.attach(new TextEncoder().encode(manifest), 'export-manifest.json', {mimeType: 'application/json', description: 'Node Comics export page manifest'});
