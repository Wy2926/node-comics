...
export function App(){
 ...
 async function showDirectory(comicId:string){try{setSourceDirectory(await comicDirectory(comicId));}catch(e){setError((e as Error).message);}}
 async function openComic(comicId:string){const entry=await continueEntry(comicId);if(entry)await openEntry(entry.id);else await showDirectory(comicId);}
