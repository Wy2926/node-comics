...
{view==='account'&&<AccountPage key={apiOrigin} api={api} account={account} notify={notify} rights={rights??undefined} testing={login.development} onEntitlements={receiveTranslationPolicy} onLogin={()=>login.setOpen(true)} onLogout={()=>{if(account)void signOut(account.id).then(()=>notify(msg("已退出账户，原图仍可继续阅读"))).catch(e=>setError(e.message));}}/>}
</main>}
</div>
