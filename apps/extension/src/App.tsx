...
{view==='settings'&&<Preferences key={`${apiOrigin}:${account?.user.id??''}`} settings={settings} setSettings={setSettings} caps={caps} cacheBytes={cacheBytes} notify={notify} onSaveApiAddress={saveApiAddress} onClearCache={()=>setConfirmAction({title:'清理本地图片缓存？',body:'所有本地原图和译图将被删除，书架和进度保留。已上传的原图和译图可从账户重新获取，未上传的原图需重新导入。',action:async()=>{await store.clearImages(copiesRef.current);setCopies(await store.readCopies());setCacheBytes(0);notify('本地图片已清理，书架与进度已保留');}})}/>}
