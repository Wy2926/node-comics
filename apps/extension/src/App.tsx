...
import {AutomaticTranslationQueue,normalizeAhead} from './reader/automatic';
...
const [cacheBytes,setCacheBytes]=useState(0);const [autoApproval,setAutoApproval]=useState(false);const autoQueue=useRef(new AutomaticTranslationQueue());const [preparingPages,setPreparingPages]=useState<string[]>([]);const currentRef=useRef(currentId);currentRef.current=currentId;
...
useEffect(()=>{stopAutomatic();},[autoScope]);
...
const autoPrice=useRef<number | undefined>(undefined);const autoConfig=useRef('');
function stopAutomatic(){autoState.current.enabled=false;autoQueue.current.reset();setAutoApproval(false);}
...
async function saveApiAddress(){
  try{
    ...
    if(base!==api.base){store.saveSession(null);setAccount(null);setUsage(undefined);setCaps(undefined);setReady(undefined);setPendingIntent(undefined);stopAutomatic();api.isCurrent=()=>false;}
...
async function prepareTranslation(intent:Intent,automatic=false,windowCurrent=()=>true):Promise<boolean>{
  if(!currentRef.current||prepareLock.current||submitLock.current)return false;
  if(!automatic){autoState.current.enabled=false;stopAutomatic();}
  ...
  if(!caps?.modes.find(m=>m.id===intent.mode)?.enabled){setError(`${modeLabels[intent.mode]}暂未配置就绪，请选择其他方式或稍后再试。`);return false;}
  ...
  try{
    ...
    if(failures.length){setError(`${failures.length} 页未能准备，其余页面可继续。${failures.slice(0,3).join('；')}`);}
    ...
    if(!pages.length){if(!automatic&&!failures.length)notify('所选页面已有结果、无字记录或正在处理，无需重复创建任务');return true;}
    assertCurrent(live);
    ...
    if(automatic){
      if(quote.unit_cost!==autoPrice.current||(autoConfig.current&&quote.config_version!==autoConfig.current)){autoState.current.enabled=false;stopAutomatic();notify('翻译配置或单价已变化，请重新开启自动翻译以确认新单价');return false;}autoConfig.current=quote.config_version;
      await submitQuote(prepared,true);
    }else setReady(prepared);
    return true;
  }catch(e){if(api.isCurrent()&&!(e instanceof StaleOperation)){if(automatic){stopAutomatic();notify(`自动翻译已暂停：${(e as Error).message}`);}else setError((e as Error).message);}}
...
async function submitQuote(prepared:ReadyQuote=ready!,automatic=false){
  ...
  try{
    ...
    refreshUsage();
  }catch(e){if(!api.isCurrent())return;if(submissionRejected(e)){clearPending();setReady(undefined);setError((e as Error).message+' 此请求未创建任务，请重新估算。');}else{setError((e as Error).message+' 提交记录已保存，请使用同一确认按钮核实，避免重复新建。');if(automatic)setReady(prepared);}if(automatic)stopAutomatic();}
...
},[autoApproval,account,settings.language,settings.translationMode,api,caps]);
function askAuto(){
  if(autoState.current.enabled){stopAutomatic();return;}
  if(!account){setLoginOpen(true);return;}
  const selectedMode=caps?.modes.find(m=>m.id===settings.translationMode);
  ...
  setConfirmAction({title:'开启自动翻译',body:`${modeLabels[settings.translationMode]} · 每页 ${selectedMode.unit_cost} 点。跟随阅读位置翻译当前页和后 ${settings.autoAhead} 页，本次翻译总页数不设上限，持续使用账户可用额度；已有结果不重复提交。关闭开关或离开阅读器后停止添加任务。`,action:async()=>{
    if(autoScopeRef.current!==autoScope)return;
    approvedAutoScope.current=autoScope;autoQueue.current.reset();autoPrice.current=selectedMode.unit_cost;autoConfig.current='';autoState.current.enabled=true;setAutoApproval(true);
  }});
}
function exitReader(){stopAutomatic();setCurrentId(undefined);}
