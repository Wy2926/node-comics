...
import {defaults,modeLabels,statusLabels,type Capabilities,type Chapter,type Job,type Mode,type Page,type Quote,type Settings,type Usage,type User,type UserQueue} from './types';
  ...
  async function poll(){
    ...
    try{
      if(ids.length){
        ...
        const downloads=[...new Map(chaptersRef.current.flatMap(c=>c.pages.filter(p=>p.ownerId===account!.user.id&&p.apiOrigin===apiOrigin).flatMap(p=>p.jobs.filter(j=>ids.includes(j.id)&&j.status==='succeeded'&&j.output_asset_id&&!p.outputBlobs[j.id]))).map(j=>[j.id,j])).values()];
...
async function recoverChapter(manual=false){
  ...
  if(!chapterId||!acc||!recoveryModeEnabled){if(manual&&!acc)setLoginOpen(true);return;}
  ...
  try{
    ...
    const pages=latest.pages.map(page=>{
      ...
      if(match.jobs.some(j=>reusableJob(j,settings.translationMode,settings.language)))restored++;
...
useEffect(()=>{
  ...
  if(account&&currentLoaded&&recoveryModeEnabled)void recoverChapter();
...
async function submitQuote(prepared:ReadyQuote=ready!,automatic=false){
  ...
  try{
    ...
    if(prepared.intent.regenerate){
      ...
      returned=[await api.rerun(prepared.rerun.jobId,`${prepared.key}:${prepared.rerun.pageId}`,prepared.quote.id,prepared.quote.total_cost)];
