...
{auth.reason==='expired'&&<div className="global-error" role="alert"><Icon name="info" size={18}/><span>{expiredMessage}</span><button className="button small" onClick={()=>login.setOpen(true)}>重新登录</button></div>}
...
{view==='account'&&<AccountPage key={`${apiOrigin}:${account?.user.id??''}`} api={api} account={account} rights={rights??undefined} testing={login.development} onEntitlements={receiveTranslationPolicy} onLogin={()=>login.setOpen(true)} onLogout={()=>{if(account)void signOut(account.id).then(()=>notify('已退出账户，原图仍可继续阅读')).catch(e=>setError(e.message));}}/>}
</main>}
</div>
