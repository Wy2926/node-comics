...
import {claimFreeReuse} from './translation/free-reuse';
...
async function prepareTranslation(intent:Intent){
 ...
 try{
  ...
  const rights=benefits.modes[intent.mode];
  const copy=copiesRef.current.find(c=>c.id===copyId);if(!copy)return;
  const requested=copy.pages.filter(p=>intent.pages.some(i=>i.id===p.id));setPreparingPages(requested.map(p=>p.id));
  let found=await matchFilePages(api,requested,intent.mode,settings.language);assertCurrent(api.isCurrent);
  const initialPlan=planTranslation(requested,found,intent.mode,settings.language,!!intent.regenerate);
  if(!intent.regenerate&&initialPlan.selected.length&&(!rights.allowed||!rights.unlimited&&initialPlan.selected.length>(rights.quota?.available??0))){
   await claimFreeReuse(api,acc.user.id,initialPlan.selected.map(page=>{const source=pageSource(page),match=source&&found.matches.get(sourceKey(source));return match?applyMatch(page,match,acc.user.id,apiOrigin):page;}),intent.mode,settings.language,rights.quota_kind,store.getBlob);assertCurrent(api.isCurrent);
   found=await matchFilePages(api,requested,intent.mode,settings.language);assertCurrent(api.isCurrent);
  }
  ...
  if(!rights.allowed)throw Error(`已恢复 ${requested.length-plan.selected.length} 页可复用结果；其余 ${plan.selected.length} 页新建任务需要 PLUS 或有效赠送额度。`);
...
{view==='account'&&<><PageTitle eyebrow="A LITTLE MAGIC FOR EVERY PAGE" title="我的账户" description="查看会员权益、可用翻译页数与到期时间。成功交付后计量，失败释放预占。"/>{account?<><div className="account-summary"><span className="avatar large">{account.user.name[0].toUpperCase()}</span><div><h2>{account.user.name}</h2><p>{authAllowed?'本地测试账户':'已登录账户'} · {account.user.role==='admin'?'管理员':'读者'}</p></div><button className="button secondary small" onClick={()=>{api.isCurrent=()=>false;store.saveSession(null);setAccount(null);setUsage(undefined);setReady(undefined);notify('已退出账户，原图仍可继续阅读');}}>退出登录</button></div><EntitlementCards data={rights??undefined}/><PlusOffer api={api} onChanged={refreshUsage}/><section className="settings-card"><div className="nc-section-heading"><div><h2>用量与反馈</h2><p className="nc-muted">查看实际消耗、逐笔记录和反馈处理进展。</p></div><button className="button primary" onClick={()=>nav('usage')}>查看用量统计</button>{account.user.role==='admin'&&<button className="button secondary" onClick={()=>nav('admin')}><Icon name="shield"/>运营管理</button>}</div></section><FeedbackInbox api={api}/></>:<div className="login-prompt"><span className="feature-icon pink"><Icon name="user" size={28}/></span><h2>准备好，一起漫游了吗？</h2><p>无需登录即可阅读原图。登录账户，开始翻译并查看额度。</p><button className="button primary" onClick={()=>setLoginOpen(true)}>登录账户 <Icon name="arrow" size={18}/></button></div>}<div className="privacy-note standalone"><Icon name="info"/><p>订阅权益由付款状态自动同步；管理员赠送权益与页数按各自期限独立生效。</p></div></>}
</main>}
</div>
