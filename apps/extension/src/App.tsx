...
import { AcquisitionCoordinator, grantImagePermissions, pauseCopies, queueCopies } from './library/acquisition';
...
import { readingViewKey, saveReadingView } from './reader/view';
import { API_BASE, API_ORIGIN } from './service';
import type { PageManifest } from './sources';
import { discoverCatalog } from './sources';
...
function viewFromHash():View {const value=location.hash.slice(1).split('/')[0];return value==='settings'||value==='account'?value:'library';}
export function App(){
...
const [accountTab,setAccountTab]=useState<AccountTab>(()=>location.hash==='#account/subscription'?'subscription':'overview');
useEffect(()=>{
  const changed=()=>{const value=viewFromHash();setAccountTab(location.hash==='#account/subscription'?'subscription':'overview');if(location.hash&&location.hash!==`#${value}`&&location.hash!=='#account/subscription')history.replaceState(history.state,'',`${location.pathname}${location.search}#${value}`);setReadingEdition(undefined);setCurrentId(undefined);setView(value);};
...
const viewKey=current?readingViewKey(library,current.id):'';
...
const nav=(value:View,tab:AccountTab='overview')=>{exitReader();setView(value);setAccountTab(tab);location.hash=value==='account'&&tab==='subscription'?'account/subscription':value;setError('');};
...
{!current&&<header className="nc-app-header"><button className="nc-brand" aria-label={msg("返回我的漫画")} onClick={()=>nav('library')}><BrandLogo/></button><nav aria-label={msg("主导航")}>{([['library',msg("我的漫画"),'book']] as const).map(([value,label,icon])=><button key={value} aria-current={view===value?'page':undefined} onClick={()=>nav(value)}><Icon name={icon} size={19}/>{label}</button>)}</nav><div className="nc-header-actions"><button className="icon-button" aria-label={msg("外观与设置")} onClick={()=>nav('settings')}><Icon name="settings"/></button><button className="nc-account-button" aria-label={msg("我的账户")} aria-current={view==='account'?'page':undefined} onClick={()=>nav('account')}><span className="nc-avatar">{account?account.user.name[0].toUpperCase():<Icon name="user" size={18}/>}</span><span>{account?(rights?.plan==='plus'?'PLUS':msg("普通用户")):msg("我的账户")}</span></button></div></header>}
<div className="nc-workspace">
{auth.reason==='expired'&&<div className="global-error" role="alert"><Icon name="info" size={18}/><span>{expiredMessage()}</span>{(current||view!=='account')&&<button className="button small" onClick={()=>login.setOpen(true)}>{msg("重新登录")}</button>}</div>}
...
{view==='account'&&<AccountPage key={apiOrigin} tab={accountTab} onTabChange={tab=>nav('account',tab)} api={api} account={account} notify={notify} rights={rights??undefined} testing={login.development} onEntitlements={receiveTranslationPolicy} onLogin={()=>login.setOpen(true)} onLogout={()=>{if(account)void signOut(account.id).then(()=>notify(msg("已退出账户，原图仍可继续阅读"))).catch(e=>setError(e.message));}}/>}
</main>}
</div>
