...
import {modeLabels,type Capabilities,type ReadingCopy,type Job,type Mode,type Page,type Quote,type Settings,type Usage} from './types';
...
type ReadyQuote={quote:Quote;intent:Intent;key:string;copyId:string;ownerId:string;apiOrigin:string;targetLanguage:string;rerun?:RerunSource};
export function App(){
...
async function openCopy(copyId:string,edition?:TranslationEdition,pageId?:string){autoState.current.enabled=false;autoQueue.current.reset();setReadingEdition(edition);setExistingOnly(!!edition);if(edition)setSettings(s=>({...s,translationMode:edition.mode,language:edition.language}));const destination=copiesRef.current.find(c=>c.id===copyId);if(destination&&pageId&&destination.pages.some(p=>p.id===pageId)){store.savePosition(copyId,destination.manifestRevision,{pageId,relativeOffset:0});setNavigationKey(n=>n+1);}setCurrentId(copyId);const copy=copiesRef.current.find(c=>c.id===copyId);if(copy?.sourceEntryId&&(!copy.discoveryComplete||copy.pages.some(p=>!p.blobKey)))try{await queueCopies([copyId],false);}catch(e){setError('原图采集未启动：'+(e as Error).message);}}
...
const pendingKey=account?`nc-library-submission:${apiOrigin}:${account.user.id}`:'';
...
const sequence=current?readingSequence(library,copies,current):[];const sequenceIndex=sequence.findIndex(c=>c.id===currentId);
...
async function prepareTranslation(intent:Intent,automatic=false,windowCurrent=()=>true):Promise<PreparationResult>{
  ...
  try{
    ...
    const prepared:ReadyQuote={quote,intent:{...intent,pages:uploaded},key:id(),copyId,ownerId:acc.user.id,apiOrigin,targetLanguage:language,rerun};
    if(automatic){
...
async function submitQuote(prepared:ReadyQuote=ready!,automatic=false){
  ...
  const storageKey=`nc-library-submission:${prepared.apiOrigin}:${prepared.ownerId}`;
...
async function askAuto(resume=false){
  ...
  setConfirmAction({title:resume?'确认自动翻译价格':'开启自动翻译',body:`${modeLabels[settings.translationMode]} · 每页 ${selectedMode.unit_cost} 点。跟随阅读位置翻译当前页和后 ${settings.autoAhead} 页，持续使用账户可用额度；已有结果不重复提交。本机会记住此账户、服务、模式与语言的选择，刷新、换漫画或下次阅读时自动继续；离开阅读器时停止添加任务，手动关闭后保持关闭。`,action:async()=>{
    ...
    setExistingOnly(false);setReadingEdition(undefined);updateAutoConsent({...autoConsents,[consentScope]:{unitCost:selectedMode.unit_cost}});autoQueue.current.reset();setAutoPause(undefined);
  }});
}
