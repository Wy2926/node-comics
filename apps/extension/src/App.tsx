...
const api=useMemo(()=>new Api(API_BASE,account?.apiOrigin===apiOrigin?account.token:'',requestPool.current),[API_BASE,account,apiOrigin]);
...
async function authenticate(){setBusy('正在登录…');setError('');try{let value:store.Session;if(authAllowed){const auth=await api.login(username.trim());value={token:auth.access_token,user:auth.user,apiOrigin};}else{if(!authConfig)throw Error('无法读取身份服务配置。');if(currentId)sessionStorage.setItem('nc-login-copy',currentId);const result=await startOidc(authConfig,API_BASE);if(!result)return;value=result;}if(value.apiOrigin!==API_ORIGIN)throw Error('登录期间服务地址已切换，请重新登录。');store.saveSession(value);setAccount(value);setLoginOpen(false);notify('已连接账户，可以开始翻译了');}catch(e){setError((e as Error).message);}finally{setBusy('');}}
