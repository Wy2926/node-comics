...
async function discover(tabId:number){
 const tab=await chrome.tabs.get(tabId);if(!tab.url||!safeImageUrl(tab.url,tab.url))throw Error(msg("请打开普通漫画网页。"));await inject(tabId);
...
export default defineBackground(()=>{
 ...
 chrome.contextMenus.onClicked.addListener(async(info,tab)=>{
  ...
  const id=crypto.randomUUID();const manifest:PageManifest={id,sourceTabId:tab.id,navigationId:source.navigationId,revision:1,title:tab.title??msg("网页图片"),url:tab.url,adapter:'context-menu',direction:'rtl',discoveryComplete:false,note:msg("右键导入的单张图片。"),items:[{id:'slot-0',url,width:800,height:1200,order:0}]};
 ...
 chrome.runtime.onMessage.addListener((message,sender,respond)=>{
  ...
  if(message?.type==='NC_IMPORT_CURRENT'&&fromDetail){void discover(sender.tab!.id!).then(result=>chrome.tabs.create({url:chrome.runtime.getURL('/reader.html?catalog='+result.id)})).then(()=>respond({ok:true})).catch(()=>respond({ok:false,error:msg("目录读取失败，请通过插件弹窗重试。")}));return true;}
  if(!trusted(sender))return;
  (async()=>{
   await localeReady();
   if(message?.type==='NC_TRANSLATE_TAB'){
    if(!Number.isInteger(message.tabId)||message.tabId<0)throw Error(msg("当前标签页不可用，请重新打开插件。"));
   ...
   if(message?.type==='NC_REGISTER_CATALOG'){
    ...
    if(!loc||loc.chapterId||catalog.sourceId!=='mangacopy'||!Array.isArray(catalog.entries)||catalog.entries.some(e=>{const l=mangaCopyLocation(e.url);return !l?.chapterId||l.slug!==loc.slug;}))throw Error(msg("来源目录无效。"));
    await chrome.storage.local.set({['nc-source:'+catalog.id]:catalog});return true;
   }
   if(message?.type==='NC_OPEN_SOURCE'){
    const data=await chrome.storage.local.get('nc-source:'+message.catalogId);const catalog=data['nc-source:'+message.catalogId] as SourceCatalog|undefined;const entry=catalog?.entries.find(e=>e.id===message.entryId);if(!entry)throw Error(msg("条目不在已确认来源目录中。"));
    const tab=await chrome.tabs.create({url:entry.url,active:false});if(tab.id==null)throw Error(msg("无法打开来源。"));await chrome.storage.session.set({['nc-managed:'+tab.id]:{url:entry.url}});return {tabId:tab.id};
   }
   if(message?.type==='NC_POLL_SOURCE'||message?.type==='NC_CLOSE_SOURCE'){
    const tabId=Number(message.tabId),data=await chrome.storage.session.get('nc-managed:'+tabId);const managed=data['nc-managed:'+tabId] as {url:string}|undefined;if(!managed)throw Error(msg("采集标签页已失效。"));const tab=await chrome.tabs.get(tabId).catch(()=>null);
    ...
    if(!tab)throw Error(msg("采集页已关闭，请重试。"));if(tab.status!=='complete'){if(tab.pendingUrl&&tab.pendingUrl!==managed.url&&!sameMangaCopyPage(tab.pendingUrl,managed.url))throw Error(msg("采集页正在跳转到其他地址。"));return null;}
    if(tab.url!==managed.url){if(!tab.url||!sameMangaCopyPage(tab.url,managed.url))throw Error(msg("采集页已跳转，请回源处理后重试。"));managed.url=tab.url;await chrome.storage.session.set({['nc-managed:'+tabId]:managed});}
    await inject(tabId);const snapshot=await chrome.tabs.sendMessage(tabId,{type:'NC_DISCOVER'});if(snapshot?.error)throw Error(snapshot.error);if(snapshot?.url!==managed.url)throw Error(msg("来源归属已变化。"));return snapshot;
   }
