...
书架卡片封面左上角显示来源，左下角以浮层标签显示最近阅读时间；不显示页数位置标签；“批量管理”支持勾选、全选当前搜索结果、取消选择和确认移除，选择不受虚拟列表渲染窗口限制。移除释放所选漫画的本机记录与文件引用，不删除云盘、源站或服务器共享图片。单项失败不阻塞其余项目。书架顶部不再显示下载管理。

支持更新检查的网站漫画会自动同步目录：持久记录上次检查和成功同步时间，每 12 小时检查到期作品；再次打开插件不会提前检查。当前适配 MangaCopy／拷贝漫画；新增条目在卡片右上角显示伸出封面的 SVG NEW 徽章，成功阅读当前漫画图片后清除，查看目录不清除。后台使用 `alarms` 权限，浏览器关闭期间暂停；失败保留旧目录和位置，仍遵守 12 小时检查间隔。检查不下载新章节图片。验收命令及边界见[目录同步验收](../../docs/validation/CATALOG_SYNC_2026_09_23.md)。
