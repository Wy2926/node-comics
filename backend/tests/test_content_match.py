...
from conftest import quota_usage, create, login_plus as login, upload, request_for_job
...
def test_repacked_page_recovers_without_mapping_upload_job_or_charge(client,png,monkeypatch):
    ...
    for item,requested in zip(result.json()['items'],pages):
        ...
        assert [j['id'] for j in item['translations']]==[request_for_job(client, auth, job['id'])]
...
def test_content_match_is_private_and_uses_input_not_output_hash(client,png,monkeypatch):
    ...
    assert match(client,auth,[source(png)]).json()['items'][0]['translations']
...
def test_content_matching_uses_mode_language_config_and_preserves_unknown(client,png,monkeypatch):
    ...
    assert match(client,auth,[source(png)]).json()['items'][0]['translations'][0]['id']==request_for_job(client, auth, job['id'])
    assert match(client,auth,[source(png)],language='en').json()['items'][0]['translations']==[]
    with session_factory()() as db:
    ...
    assert result['display_translations'][0]['id']==request_for_job(client, auth, job['id'])
...
def test_completed_submission_cache_hits_by_content_without_second_charge(client,png,monkeypatch):
    ...
    assert receipt['result']['asset_id'] == job['output_asset_id']
    from conftest import request_record
    assert request_record(client, auth, receipt['id']).job_id == job['id']
    assert quota_usage(client, auth)==before
