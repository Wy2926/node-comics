...
from app.jobs import create_batch, create_job, create_quote, locked_user
...
def test_two_devices_match_miss_then_confirm_distinct_quotes_share_one_paid_job(pg, monkeypatch):
    ...
    body = FilePageMatchRequest(pages=[identity], mode="redraw", target_language="zh-Hans")
    with session_factory()() as db:
        source = upload_file_page(db, pg["owner_id"], pg["png"], identity)
        db.commit()
        source_id = source.id
    prices = []
    # Both devices observe the miss and independently obtain a quote BEFORE either confirms.
    for _ in range(2):
        with session_factory()() as db:
            assert match_file_pages(db, pg["owner_id"], body)["items"][0]["jobs"] == []
            price = create_quote(db, db.get(User, pg["owner_id"]), [source_id], "redraw", "zh-Hans")
            prices.append((price.id, price.total_cost))
    ...
    with session_factory()() as db:
        ...
        assert db.get(User, pg["owner_id"]).reserved == 8
    calls = []
    def provider(*args):
        calls.append(1)
...
def test_completion_holding_job_lock_allows_user_locked_reuse_receipt_and_batch_fks(pg, monkeypatch):
    ...
    job_id = new_job(pg)
    with session_factory()() as db:
        price = create_quote(db, db.get(User, pg["owner_id"]), [pg["asset_id"]], "redraw", "zh-Hans")
        quote_id, upper_bound = price.id, price.total_cost
    admit_jobs()
    token = admission_token_for(job_id)
    ...
    def pause_under_real_worker_job_lock(db, *args, **kwargs):
        ...
        # Inserting the output now waits on the other transaction's User lock.
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        ...
        try:
            ...
            with session_factory()() as db:
                ...
                resume_finalize.set()
                def worker_waits_for_user():
                    with pg["administration"].connect() as observer:
                        return observer.scalar(text("SELECT cardinality(pg_blocking_pids(:pid)) > 0"), {"pid": backend_pid["value"]})
                wait_until(worker_waits_for_user)
                ...
                batch = create_batch(db, user, quote_id, upper_bound, "reuse-during-finalize")
...
def test_single_page_and_batch_competing_on_identical_bytes_keep_both_request_receipts(pg):
    ...
    with session_factory()() as db:
        ...
        db.commit()
        alias_id = alias.id
        price = create_quote(db, db.get(User, pg["owner_id"]), [alias_id], "redraw", "zh-Hans")
        quote_id, upper_bound = price.id, price.total_cost
    barrier = threading.Barrier(2)
    def single():
        with session_factory()() as db:
    ...
    def batch():
        with session_factory()() as db:
            user = db.get(User, pg["owner_id"])
            barrier.wait(timeout=10)
            group = create_batch(db, user, quote_id, upper_bound, "batch-device")
