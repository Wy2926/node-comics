...
权益依据 [会员与额度设计](../../docs/MEMBERSHIP_AND_QUOTAS.md)，使用服务端实际账户与订阅接口。结账直接跳转 Stripe 官方托管 Checkout，订阅管理使用 Customer Portal；页面本身不收集银行卡。Stripe 未开启时，账户显示订阅服务不可用；不会伪造已开通订阅。
...
2026-09-20 支付更新：结账与订阅管理统一跳转 Stripe 托管页面，移除站内旧结账／取消接口。5 项测试、类型检查和 105 页构建通过；真实 Stripe 付款仍未验收，配置见 [Stripe 支付](../../docs/STRIPE_BILLING.md)。
