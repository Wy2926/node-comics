...
function OrderDetail({id, onClose, onUnauthorized, onChanged}: {id: string; onClose: () => void; onUnauthorized: (message: string) => void; onChanged: () => Promise<void>}) {
  ...
  return <BillingDialog title="订单详情" onClose={onClose} busy={reconciling}><div className="billing-order-detail" aria-busy={loading || reconciling}>
    ...
    {!data ? <p className="loading" role="status">{loading ? '正在读取订单…' : '暂时无法读取订单详情。'}</p> : <>
      ...
      <h3 className="detail-heading">退款与争议</h3>
      {data.refund_summary && <><dl className="detail-meta"><dt>平台退款累计</dt><dd>{money(data.refund_summary.reported_total, data.refund_summary.currency)}</dd><dt>已记录成功退款</dt><dd>{money(data.refund_summary.recorded_succeeded_total, data.refund_summary.currency)}</dd></dl>{!data.refund_summary.details_complete && (data.refunds?.length > 0 || ['refunded', 'partially_refunded'].includes(data.order.status) || (data.refund_summary.reported_total ?? 0) > 0) && <p className="muted">退款明细尚未与平台累计金额完整核对。未知金额以“—”显示；历史通知未提供的明细不会推算。</p>}</>}
      {(data.refunds?.length || data.disputes?.length) ? <Table heads={['类型 / 平台编号', '金额', '状态', '原交易', '发生 / 最近同步']}>{[...(data.refunds || []).map(row => ({...row, kind: '退款'})), ...(data.disputes || []).map(row => ({...row, kind: '争议'}))].map(row => <tr key={`${row.kind}:${row.id}`}><td>{row.kind}<small><code>{row.external_id}</code></small></td><td>{money(row.amount, row.currency)}</td><td>{billingStatus(row.status)}</td><td><code>{row.transaction_id}</code>{row.event_id && <small><a className="text-link" href={href('billing-events', {q: row.event_id})}>查看支付事件</a></small>}</td><td>{time(row.occurred_at)}<small>{time(row.synced_at)}</small></td></tr>)}</Table> : <p className="muted">暂无已记录的逐笔退款或争议。</p>}
      ...
      {data.subscription && <p><a className="text-link" href={href('subscriptions', {q: data.subscription.id})}>查看订阅账单、授权期与额度桶</a></p>}
