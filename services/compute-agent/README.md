...
2026-09-19：本目录为旧阶段协议代理，仅用于升级前任务排空。新整页节点直接使用 [classic-engine](../classic-engine/README.md)，无需运行本代理。以下为旧接入方式。

此历史代理负责向总中心领取图像阶段、续租、获取授权输入、调用独立图像引擎并提交结果。代理本身不执行 OCR、抹字或嵌字，也不访问数据库、R2 凭据或文本供应商密钥。仓库新增的整页节点见 [classic-engine](../classic-engine/README.md)。
