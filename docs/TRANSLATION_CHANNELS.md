...
插件提供内置 NodeLane 渠道与可配置的 manga-translator-ui（MTU）渠道。阅读器和网页原位翻译共用当前渠道；支持添加多个 MTU 服务配置，每次只选择一个。MTU 使用用户自己的服务与算力，无需 NodeLane 账号，不使用官方额度。本次没有新增自定义 HTTP 协议。
...
2026-09-25 的 Chromium 实测通过以下检查：无 NodeLane 账号的原位翻译；非激活宿主页发送 MTU 表单；服务器超过 35 秒不返回响应；停止 service worker 后以执行上下文内存标记消失核实重启，宿主请求继续且 HTTP 受理次数保持 1；恢复后显示译图；刷新源页复用结果；空闲宿主自动关闭。预算设为 `cacheLimitMb=0` 时，两个独立扩展上下文均可读取有界内存结果，持久译图字节为 0；清理缓存后两端拒绝恢复旧字节，也不自动重译。可重复脚本将截图与脱敏检查报告写入 `artifacts/translation-channel-host/<run-id>/`。
...
2026-09-25 在用户现有本机服务完成真实登录与合成图翻译，返回可解码的 800×1100 PNG，并人工核对英文已替换为中文。登录 JSON 字段与原协议一致；无法据此重现用户先前的“用户名和密码”错误。审查修正了响应格式异常被误报为凭据错误的问题：无效响应单独提示，成功响应要求修改初始密码时优先提示修改密码。

真实补测还暴露了执行锁交接竞态：宿主过早确认接收时，后台可能把尚未登记执行锁的请求误判为中断，HTTP 调用为 0。原模拟锁同步登记等待状态，遗漏该窗口。延迟锁登记回归在旧实现的宿主与阅读器路径均失败；修复后通过。最终真实验收 4 项通过：设置页登录、凭据隔离、一次 HTTP 请求约 20 秒返回并显示译图、刷新复用且不再提交；另有 41 项相关单测及 8 项模拟宿主检查通过，包括 35 秒无响应期间重启后台、零预算结果共享与清理。上述证据限于本机 Chromium、该服务默认配置及测试样本，不代表所有引擎、真实 Firefox 运行或公开部署已通过。
