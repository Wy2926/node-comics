...
2026-09-19 新节点已实现整页承接：执行位按未释放整页租约计数，线程、设备和缓存由节点本地配置；普通配置直接限制后续领取。新节点使用 [classic-engine](../services/classic-engine/README.md)，消息与配置见 [v2 实施说明](COMPUTE_V2_IMPLEMENTATION.md)。下文保留旧阶段代理的配置说明，仅用于旧任务排空，不适用于新节点的执行语义。
