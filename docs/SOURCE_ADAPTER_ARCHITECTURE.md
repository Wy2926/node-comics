...
更新：2026-09-23。状态：**代码解耦已实现，按新库基线直接替换旧模块，无迁移、转发入口或旧逻辑回退**。本文记录采用的结构、契约和依赖约束；本轮验证见第 10 节及[网站图片适配](SITE_ADAPTERS.md)。

## 1. 设计结论

采用“公共执行层 + 通用适配器 + 独立站点适配器”。通用适配器和站点适配器使用同一套契约；站点适配器按站点分别放在 `sites/<site-id>/`，互不导入。站点匹配、URL 解析、图片发现、目录解析、原始标签展示、特殊读取和观察规则都由对应站点持有。

公共层负责选择适配器、生命周期、消息校验、权限、资源登记、有限轮询和结果交付。阅读器、书库、翻译调度和后端不包含站点名称分支、域名、站点选择器或 URL 拼装规则。

目录能力仍按站点提供。MangaCopy／拷贝漫画和 Comix 支持自动目录同步与封面更新提示；Comix 另通过网络适配器完成无标签页接入。
...
├── registry/
│   ├── definitions.ts          # 按通配约定自动收集 definition
│   └── pages.ts                # 按通配约定自动收集 page 工厂
├── core/
...
每站提供 `definition.ts` 与 `installation.json`；`page.ts`、`network.ts`、`image.ts` 按实际能力独立提供。网络站点无需空的页面工厂，混合站点可分别用 HTTP 和 DOM 读取目录／页清单。标准 `<img>` 显示和画布覆盖层属于 `inline/` 的通用展示实现；其中不保留站点选择器和站点身份判断。通用按钮样式属于 UI，站点只提供挂载位置，公共运行时负责点击行为与清理。

### 无标签页网络通道

`contracts/network.ts` 的 `catalog` 和 `pages` 是独立可选操作，运行时按操作选择通道。`registry/networks.ts` 按 `sites/*/network.ts` 收集解析器；`registry/images.ts` 独立收集 `image.ts` 的请求头／解码能力。目录使用网络读取不强制章节也使用网络，图片解码不要求存在网络目录能力。已选通道失败明确报错，不隐式切换通道。

网络与 DOM 适配器都返回 `SourceSnapshot`，共用 `core/pages.ts` 校验；经过 `runtime/manifests.ts` 登记后才成为可读取清单。`PageManifest.pageContext` 仅为页面授权保存真实 `tabId`、`navigationId`，HTTP 清单不构造负标签页 ID 或虚假导航。内容脚本的观察版本与持久清单身份分开。

`readSourceImage({manifestId,pageId,expectedUrl}, signal)` 向页面服务交付 Blob，内部完成清单核验、权限、获取和站点解码；`processing` 只在来源模块内部传递。`image-headers.ts` 为精确 URL 设置扩展来源的请求头规则，所有扩展 HTTP 图片读取（含无特殊头的读取）按该 URL 共享 Web Lock；同 URL 并发请求不会互相覆盖，响应消费完成或取消后清理规则，后台恢复只清理未被活跃读取持有的规则。站点解析、图片规则、业务缓存策略各自独立。

目录读取只接受调用方传入的 `previous`，不写入另一份已选上传状态。`comics/application/website-catalog.ts` 从书库已接受的目录取出该快照；用户导入、后台当前页导入和定时刷新均走这一入口。新观察经书库的代次、租约和时间检查后才能成为下次读取的基准。`nc-source:` 仅用于页面采集授权，不参与网络适配器的上传选择。

`ui/ComicSites.tsx` 是公共漫画网站目录页，经 `sources` 公共入口读取注册元数据，没有具体站点组件。链接导入调用应用服务，权限申请在来源服务中于用户手势内启动；页面不判断网络通道、不读取安装权限，也不展示取数实现细节。
...
- 具体站点只允许在自己的目录内部互相导入；其他目录（包括 `registry/`）禁止直接 import、export 或动态 import 具体站点。注册表只使用构建期 `import.meta.glob` 的统一文件约定，不出现任何站点名称或专属路径。依赖检查强制执行此约束。
...
站点按 `definition.ts`、可选 `page.ts`／`network.ts`／`image.ts` 自动收集并随扩展静态打包。新增站点只修改自身目录，无需修改注册表。每站的 `installation.json` 是构建和运行时共同使用的安装权限元数据；构建工具统一读取 JSON，不导入站点可执行代码。站点专属图标、协议实现和验收脚本也归站点所有。不增加远程脚本、动态安装适配器或运行时文件夹扫描。契约测试校验元数据、页面工厂与安装权限一致性。
...
interface SourceDefinition {
  ...
  capabilities: {
    importable?: boolean;      // 专门适配的网站才能显式开放漫画导入
    pages: boolean;
    inline: boolean;
    catalog: boolean;
...
HTTP 图片只在已登记范围内获取。页面内图片在编码前后核实当前版本，超时、取消、离页、替换画布或导航后拒绝旧请求。原图遵守已有单图尺寸／字节限制，缩略图使用有界预算。编码结果不写入默认日志；图片物化后保存内容哈希和 Blob 键，失效的页面句柄不成为可重试的网络原图地址。

重复 URL 可属于不同页槽，页序身份与字节去重分开。格式修订或来源版本改变不能把旧译图贴到新内容上。

通用图片枚举先保留仍存在的元素页槽，再按 URL 和出现顺序匹配上一份清单中被替换的元素；相同图片重新挂载时保留页槽和顺序，重复 URL 仍保留不同 ID。临时句柄仍绑定具体元素及版本。资源登记按页槽和资源键索引查找，缩略图按来源版本缓存，总缓存不超过 2 MB；预算不足时跳过编码，版本失效或导航时释放。

### 5.4 只读目录与单来源模型

`SourceCatalogSnapshot`、`SourceEntry`、`SourceGroup` 位于 `contracts/source.ts`。适配器提供稳定条目、URL、顺序、只读原始标签、分组引用与可选的 `sequenceId`；分组可用 `parentId` 表示有界树，目录可提供 `defaultEntryId`。

核心不包含标准章节类型、正文／番外角色、分类映射或编辑 schema。一部来源资源直接映射为一个 Comic，条目为 Entry；同一条目多分类引用也只有一份进度。目录缺少默认入口时先显示只读目录；仅同一显式阅读序列允许连续阅读，不通过分类或标题猜测。

公共校验检查结构、唯一 ID、顺序、组引用、循环、深度和资源上限；对应定义核验每个目录／条目 URL、来源身份与所属资源。没有全局作品 ID、用户排除项、手工归属或修订历史。来源快照不提供任意 HTML 或可执行行为。

## 6. 选择、回退和生命周期

1. 公共入口先校验普通 HTTP(S) 页面地址，再让站点定义进行纯 URL 识别。
2. 恰好一个站点匹配时使用该站点；没有站点匹配时使用 `generic` 供原位翻译；漫画导入直接拒绝。多个站点同时匹配是配置冲突，明确报错，不依赖数组先后决定结果。
3. 站点可将已知但不支持的路径识别为 `other` 并返回 `unsupported`；明确未认领的路径才走通用适配。能力缺失和解析失败均不触发隐式通用回退。
4. 内容运行时创建会话，调用所需能力并统一校验输出。专用网站的网页发现与网页翻译必须使用同一个选择结果和页序定义，不能分别猜站点。
...
通用适配的首轮能力仅用于网页原位翻译：发现已加载的大图，读取 `currentSrc/src`，网页翻译支持当前已支持的页面内图片；不盲目读取所有 `data-*`，不自动采集任意 canvas、CSS 背景或 iframe。新渲染类型需要单独验证和明确的资源／展示契约。
...
| `mangacopy` | 容器页槽 + 已验证内联数据；HTTP 原图 | 来源只读目录、标签与分组 | 有界等待完整数据，不自动滚动 |
| `comicpash` | Comici 已渲染 canvas、页槽与版本 | 本次仍无 | 快照；用户翻页触发新目标，不自动翻完整章 |
| `comix` | 网络章节清单、HTTP 原图及切片还原 | 全量分页、每话一条、12 小时同步 | 无标签页直接读取、核对完整页数 |
...
验收必须覆盖：未知网站走通用；已匹配站点等待／失败不误抓广告；站点域名与伪造相似域名隔离；镜像同页与跨作品跳转区分；部分页槽与重复 URL；目录分组引用、自定义标签与阅读序列；暂停／取消／SPA 切换；失效句柄与来源替换；原图恢复、画布变换对齐和权限不扩大。

实现检查在扩展目录运行 `npm run check`、`npm test`、`npm run build`，再执行 `verify_popup.mjs`、`verify_inline_translation.mjs`、`verify_comicpash.mjs` 与 `verify_website_source_lifecycle.mjs`。真实站点、隔离样本、真实模型效果分别记录；测试数量以实施后的运行结果为准。具体执行结果单独记录，不将此前设计检查或历史真实网页记录计入本轮验证。

## 9. 已确定与后续范围

本设计采用函数组合和显式注册，通用适配与站点适配使用统一契约；所有现有站点逻辑按目录隔离；目录、身份、安装元数据和观察规则一起解耦。无需进一步选择框架或增加运行时依赖。

本轮交付包括代码解耦、依赖边界检查、站点与公共运行时测试。iframe／CSS 背景支持、自动翻章和远程适配分发不在本轮范围。

## 10. 实现与验证记录

- 公共纯定义入口：`sources/index.ts`；页面入口：`sources/page.ts`。`registry/definitions.ts` 与 `registry/pages.ts` 只装配定义／工厂。DOM 站点及通用适配使用相同会话契约，网络操作与图片能力独立注册，站点目录包含范围说明与独立测试。
- `SourceSnapshot.items` 使用 `DiscoveredPage.resource` 的 HTTP／页面资源联合类型。内容运行时将页面逻辑引用转换成不可自选的临时句柄，消息快照使用 `PageSnapshot`；持久页面记录只保存原图内容哈希、Blob 键或可恢复 HTTP 来源，不保存页面句柄。该消息封装用于当前运行时授权，不提供旧数据读取或格式回退。
...
- 目录快照不携带 `workId` 或用户排除项；公共校验核实唯一 ID、组引用、顺序、边界和逐条 URL 目录归属。MangaCopy 将原始标签展示为 `suggestedKind`，书库不再解析站点语言标签。
- 安装权限仍只有原有 MangaCopy／copy4000 来源授权；新增注册不扩大安装权限。真实权限弹窗、真实模型效果与隔离浏览器测试分别看待。

当前验证见[单来源验收](validation/SIMPLE_READING_2026_09_23.md)。临时扩展、隔离网页和模拟 Google 服务用于验证协议与交互，不代表真实模型效果、Google 授权或在线站点均已重新验收。

来源边界修正与当前验证见[来源边界验收](validation/SOURCE_BOUNDARIES_2026_09_23.md)。
