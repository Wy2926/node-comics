...
- 前端每个阅读器／网页翻译上下文的原图上传并发固定为 5，不提供设置入口，与服务器执行资源独立；阅读器图片恢复及原图／译图下载不设客户端并发上限，仍遵循有限阅读窗口。`GET /v1/me/queues` 返回常规、重绘两种队列的容量、实时名额及当前占用；服务端通过 `FREE/PLUS_QUEUE_CAPACITY`、`FREE/PLUS_REALTIME_SLOTS`、`FREE/PLUS_SCHEDULER_WEIGHT` 分别配置套餐。
