...
普通点击不会自动翻译。可选“边读边译”开启时确认模式、单价及使用账户可用额度，默认窗口为当前页和后 10 页，提前量可设 0–50 页，不限制本次翻译总页数。关闭后停止创建新任务，已提交的任务由后端继续。主动生成新版本仍需确认报价；结果待核实任务保留原提交记录，不自动重发。
...
- 打开已导入漫画后，用当前账户、服务、翻译方式和目标语言匹配任务，再授权下载结果。查询和恢复不创建新任务、不再次扣点；原图仍保存在本机，不为匹配上传整卷。结果按有效配置版本区分，已删除、过期或物理缺失的图片不能复用；默认保留期仍为 7 天。书架与阅读位置没有云同步。
...
- 前端“传输并发”默认 2，可调 1–10，用于上传及下载请求；服务端“账户队列并发”独立配置，默认 2，上限由 `USER_QUEUE_MAX_CONCURRENCY` 限定且不超过 10。`GET/PUT /v1/me/queue` 查询／修改账户设置，管理员可用 `/v1/admin/users/{id}/queue` 配置指定用户。
- 服务端按用户轮转授予持久化执行名额，每轮每用户取一页，所有批次、单页任务和两种翻译模式共享该用户限额。运行中和已授予名额的排队任务都计入占用；调低并发不打断已运行请求。Worker 还受自身进程数及供应商并发上限限制，设置 10 不保证同时处理 10 页。
- 本次不回填旧文件映射，也不猜测旧数据的页索引；未建立新标识的数据不参加文件匹配。升级后 API、dispatcher、worker 与前端须配套使用；数据库启动迁移建立新表，不要求清空现有服务数据。
