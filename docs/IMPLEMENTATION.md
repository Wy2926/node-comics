...
打开 [阅读器](http://127.0.0.1:5173/)；[API 文档](http://127.0.0.1:18088/docs)。新账户为普通用户，每日 100 页常规翻译；PLUS 常规不限量，每会员月 300 页重绘。两档并发 2。开发用户名 admin 可管理会员与周期补偿；管理员角色不自动获得 PLUS。运行前确认使用新数据库，详见[会员额度实现](MEMBERSHIP_IMPLEMENTATION.md)。

常规单页直接提交，AI 重绘、批量和主动重译确认页数上限。自动翻译首次确认模式权益及持续提交，默认窗口为当前页和后 10 页，可调 0–50 页；根据额度和待处理队列缩小新增范围。额度用尽暂停，刷新或获得有效赠送后在同意范围内继续；权益类型变化需重新确认。未知提交保留原幂等编号，核实前停止新增。
...
- `POST /v1/images` 上传时携带成对的 `file_hash`、`page_index`；`POST /v1/file-pages/match` 按页批量查询（每次最多 100 页）。映射按账户私有，同一文件同页重复上传返回仍可用的原图；同一标识对应不同图片字节返回冲突。
- 打开已导入漫画后，用当前账户、服务、翻译方式和目标语言匹配任务，再授权下载结果。查询和恢复不创建新任务、不再次扣页数；原图仍保存在本机，不为匹配上传整卷。结果按有效配置版本区分，已删除、过期或物理缺失的图片不能复用；默认保留期仍为 7 天。书架与阅读位置没有云同步。
...
- 前端传输并发默认 2，可调 1–10。后端 `FREE_CONCURRENCY` / `PLUS_CONCURRENCY` 默认均为 2；`GET /v1/me/queue` 与管理员 GET 接口只读，用户不能提高执行上限。
- 服务端按用户轮转授予持久化执行名额，每轮每用户取一页，所有批次、单页任务和两种翻译模式共享该用户限额。运行中和已授予名额的排队任务都计入占用；调低并发不打断已运行请求。Worker 还受自身进程数及供应商并发上限限制，套餐并发是上限，实际处理量还受服务容量限制。
- API、dispatcher、worker 与前端配套更新。新计费基线需要新数据库，不兼容旧计费数据；本次未清空原产品数据库和图片存储。
