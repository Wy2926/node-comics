...
`config_version` 包含完整有效配置，不只模型名：重绘包括供应商配置、参数、提示词版本和点数；常规翻译包括图像引擎版本、文字模型/提示词、重试预算和计价配置；模型、字体与渲染变更须由接入引擎纳入其版本标识。当前实现中修改某些供应商超时、并发或显示配置也会改变快照。换模型、模式、目标语言或账户不会作为同一个成功结果缓存。
