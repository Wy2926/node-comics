...
后端按内容、模式、语言与有效配置复用已完成结果，访问记录按用户隔离；过期或已删除的访问记录不能复活。原图和最终译图保存在私有 R2，默认无限期保留，不随普通客户端缓存清理删除。规则见[阅读计划契约](READING_TRANSLATION_CONTRACT.md)、[译图共享](RESULT_SHARING.md)与[对象存储](OBJECT_STORAGE.md)。不提供漫画译本卡片、旧原文件或版本选择。
