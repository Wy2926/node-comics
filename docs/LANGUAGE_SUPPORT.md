...
仓库不再附带图像引擎、字体、字典或其测试记录。产品允许选择某目标语言，不代表已有可用节点，也不代表 OCR 能可靠识别该源语言。接入引擎需要自行验证源语言 OCR、目标字体、换行、缺字和排版边界，并准确报告能力。参见[节点配置](NODE_CONFIGURATION.md)和[交互协议](COMPUTE_PROTOCOL.md)。
