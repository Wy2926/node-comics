...
- 后台预建节点并绑定稳定 `resource_id`，签发独立凭据（仅摘要入库），设备只可报告自身能力；同主机引擎还使用共享设备锁文件避免多个进程争抢同一 GPU。锁文件无用户图片。
...
- 节点配置、第二台机器和 CUDA 部署见[计算节点说明](../services/compute-agent/README.md)。每节点阶段执行位由服务端配置（1–32）；代理按配置并发领取，单设备模型保留安全串行锁。引擎以最多 3 个在途阶段实现 CPU 后处理／Qt 排版与设备推理跨页重叠，见[引擎流水线](ENGINE_PIPELINE.md)；执行位数量不代表模型副本数或 GPU 并行推理吞吐。
...
后台添加、版本化通用配置、语言资源启动补全、性能文件与热更新协议见[节点配置说明](NODE_CONFIGURATION.md)，NVIDIA 实测见[CUDA 验收](NVIDIA_GPU_VALIDATION.md)。节点不能自注册创建身份、修改执行位或自行解除停用。未应用当前配置的节点不领取新阶段；更新时排空当前阶段再应用，失败报告可见且停止新领取。新安装基线为 `shared_0001`，已合并旧迁移，不兼容旧数据库和共享注册 Token。
