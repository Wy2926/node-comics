...
- OCR／遮罩检查点及译文存 PostgreSQL；中间清理图只存引擎内存 LRU，默认 256 MiB／15 分钟 TTL。换节点或缓存丢失时重新抹字，复用已保存译文。
- 已完成 OCR、等待后续渲染的预存页数有水位限制，防止远远超前生成中间状态；实时页可越过此预存水位。
- 设备通过稳定 `resource_id` 去重注册；同主机引擎还使用共享设备锁文件避免多个进程争抢同一 GPU。锁文件无用户图片。
- 计算代理只允许从本控制服务获取输入，验证 SHA-256；返回结果再次校验版本、像素、尺寸、输出类型和租约。
