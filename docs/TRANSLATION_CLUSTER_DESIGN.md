...
2026-09-19 新计算节点已按[计算协议](COMPUTE_PROTOCOL.md)实现整页租约、整页接单名额、R2 短期授权直读和独立文本池，节点自行组织内部计算。见 [v2 实施说明](COMPUTE_V2_IMPLEMENTATION.md)。下文的三阶段图像领取、水位、缓存授权和阶段公平计量仅描述升级前任务；新页按整页名额占用时间计量，不使用旧图像水位。用户权益、实时优先及对象保留规则继续有效。
