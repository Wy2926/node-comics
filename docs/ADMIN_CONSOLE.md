...
| 系统设置 | 上传并发、空闲／总超时、占位有效期和反馈速率／突发／每日预算；统一持久化、版本冲突保护，保存后供新请求读取 |
| 订阅套餐 | 套餐权益版本、月付／年付报价、草稿、发布与停售；发布向 Stripe 核验，新价仅影响新订阅 |

`<ADMIN_WEB_PATH>#billing` 管理不可变套餐与报价。金额按币种最小单位填写；新报价默认草稿，未绑定 Stripe 时不能发布。调价复制为新报价，修改权益新建版本；旧订阅及在途结账继续原价。完整表结构、状态和年度额度规则见[订阅设计](STRIPE_BILLING.md)。
...
容器构建 `docker build -t node-comics-backend:local backend` 自动完成前端构建并复制到最终 Python 镜像。数据库仅保留 `subscription_0001` 全新空库基线，包含执行机字段、任务索引、节点身份和系统设置，不升级旧库。控制工作进程需使用同版本代码以记录执行机。
