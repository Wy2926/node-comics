...
实测阅读页最初可能只有 3 张图。源站按实际滚动事件追加槽位，且要求停留在漫画容器前段；跳到占位尾部、反复滚到同一点或一直下滚均可能停滞。2026-09-15 已改为先直接滚到当前范围中部；中部未发现增长时，临时返回首图触发加载，再把滚动条放回中部。弹窗识别与后台条目发现共用完整性轮询，细节与新验收见[阅读目录与插件译本](READER_DIRECTORY_AND_EDITIONS.md)。发现阶段保留计数，停滞和总时限分别约束，取消／暂停会结束等待。发现完整后逐页获取字节、校验解码与尺寸并持久化。
