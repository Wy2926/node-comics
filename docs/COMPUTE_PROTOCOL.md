...
新常规任务已使用 page / text 独立租约，图像节点从 R2 授权直读。旧 analyze / inpaint / render 接口暂保留供升级前任务排空，不能用于 v2 页。本文只调整常规翻译计算协议，AI 重绘仍使用中心独立执行池；代码完成不代表线上已切换。
...
- 节点名额与账户普通 3 / PLUS 10 页在途上限分别计算；中心文本池的容量、供应商 RPM 和调用时限也独立保留。
...
| 完成摘要、图片校验、R2 持久化和终态事务 | 保留 | HTTP 成功、节点声称上传成功均不等于用户可用结果 |
...
| `POST /leases/{id}/complete` | 提交最终结果或固定错误码，二选一；持久化确认后返回稳定终态回执 |
...
- `complete` 先冻结结果摘要；相同结果重发取回原回执，冲突内容拒绝。结果确认丢失时保留本地结果并重试交付，不能重新计算或把任务当成已释放；心跳也可返回已持久化的完成回执。
...
## 7. 结果交付：先保留中心验收，再考虑 R2 直传

首版保留最终图片、抹字掩膜和字形掩膜提交中心：校验输入 / 版本、PNG 可解码、尺寸、字形掩膜，以及两掩膜并集外 RGB 不变；恢复原图 alpha 后生成最终字节，写入不可变内容对象，最后确认资产、终态、额度和租约。分析检查点继续限制在 4 MiB；图片解码与网络 I/O 在短数据库锁之外。

这些校验保证数据格式和允许区域外的像素完整性，不证明 OCR 或翻译语义正确。节点返回 HTTP 200、对象存在或上传回执都不能单独触发结算。无文字、失败和成功的权益处理继续使用中心现有业务规则。

**输出直传 R2 可后续优化，首版不同时引入。** 当前中心还要读取原图、检查掩膜并恢复 alpha；仅给节点一个最终对象 PUT URL 并不能省掉验收，反而增加临时产物和提交恢复状态。
