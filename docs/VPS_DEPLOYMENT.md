...
- Chrome 包 6,804,848 字节，SHA-256 `19ff0d3d967ee68ff2531788598936e49a5c5ca14ab607af15cc520481c807cb`；Edge 包 6,804,847 字节，SHA-256 `746ea9cc4cc7fa8f37cafe1ca636052fbceb34f9debfbac2258e94006b42b104`。
- 五语更新日志及 RSS 新增 2026-09-25 的 0.3.0 条目，说明自建 manga-translator-ui 渠道、阅读器与原位翻译共用渠道、长请求稳定性和译图缓存／导出改进。
- API 镜像 `node-comics-website:030-20260925t025543z` 已健康运行，仅替换官网静态文件与下载目录；工作进程、业务环境和代理配置保持不变。备份位于 `/opt/nodelane/node-comics/extension-030-20260925T025543Z/backup/`。
- 官网 13 项测试、下载路由 10 项测试及 110 个静态页面构建检查通过。公网 Chrome 实际点击两个下载，文件名、大小及 SHA-256 均匹配；五语下载页、更新日志和 RSS 核验通过，历史下载路由继续返回有效重定向。源站 15 个相关资源与本地构建逐字节一致；公网 HTML 含 CDN 邮件保护与校验脚本，按更新区域核对，RSS 按完整字节核对。
- 本次插件打包前 889 项单测通过、1 项跳过，类型／模块／国际化检查通过；未重新执行真实翻译或 Google 授权，未提交商店。产物与脱敏发布记录位于 `artifacts/release-0.3.0/`，商店包位于 `artifacts/release-0.3.0-stores/`。
