...
调研日期：2026-09-14。历史选型记录；2026-09-16 已取消下文当时建议的页成本上限，当前文本仅计量成本，自动重试受次数、处理时限和 RPM 约束，参见 [运行说明](CLASSIC_IMPLEMENTATION.md)。用户确认：保留现有 AI 重绘，增加常规漫画翻译；文本可交给 LLM，允许 LaMa 局部抹字修复；低成本 LLM 的费用保护可简化为预算内自动重试。本轮按用户要求不评估许可证。
...
2026-09-19 状态：仓库不再附带图像引擎；中心调度与接入协议见[常规翻译说明](CLASSIC_IMPLEMENTATION.md)。以下保留调研当时的方案状态及待验证建议。
